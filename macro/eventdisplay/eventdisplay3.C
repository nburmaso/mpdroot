...
void eventdisplay3(const char* sim_file = "$VMCWORKDIR/macro/mpd/evetest.root", const char* reco_file = "$VMCWORKDIR/macro/mpd/mpddst.root", bool is_online = false)
{
    gDebug = 0;
...
void SetTasks(MpdEventManager* fMan)
{
	...
	MpdMCPointDraw* CpcPoint = new MpdMCPointDraw("CPCPoint", mcPointColor, kFullSquare);
    fMan->AddTask(CpcPoint);
	...
	// draw CPC towers
    MpdCpcTowerDraw* MpdCpcTower= new MpdCpcTowerDraw("MpdCpcTower", 0.0001, kTRUE, 1);
    fMan->AddTask(MpdCpcTower);
