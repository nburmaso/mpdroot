...
Double_t Zdc_NPb_in_module = 48;//1-12 + 36(6sect)
...
Double_t Zdc_NTyvec_in_module = 48;//1-12 + 36(6sect)
Double_t Zdc_Tyvec_X_size = 7.4; //(15.-2.*0.1)/2.
Double_t Zdc_Tyvec_Y_size = 7.35; //(15.-2.*0.15)/2.
Double_t Zdc_Tyvec_Z_size = 0.22; //0.44/2.


//Subsections - section 1 -> Fe(0.004cm)+12*(Pb(0.613cm)+(Sc+Tyvec)(0.44cm))
//Subsections - section 1 -> Fe(0.04cm)+12*(Pb(0.61cm)+(Sc+Tyvec)(0.44cm))

//Fe slices
Double_t subsect_Zdc_Fe_Z_size = 0.02; //0.04/2.

//Pb slices
Double_t subsect_Zdc_NPb_in_module = 12;
Double_t subsect_Zdc_Pb_Z_size = 0.305; //0.61/2.

//Tyvec slices
//Double_t subsect_Zdc_NTyvec_in_module = 12;//sect 1
...
void create_rootgeom_zdc_oldnames_7sect_12subsect_noSlots_v1() {
    ...
    const TString geoDetectorName = "zdc_oldnames_7sect_12subsect_noSlots";
...
void FillModule(TGeoVolume *mother_volume,TGeoVolume *mother_volume_hole) {
    ...
    TString subsect_Fe_name = "subsect_Fe";
    TString subsect_Pb_name = "subsect_zdc01a";
    //TString SensitiveVolume_name = "zdc01s";
    //TString Tyvec_name = "Tyvec";
    ...
    if(Tyvec_medium) TyvecV->SetMedium(Tyvec_medium);
     ...
     TGeoShape *subsect_FeS = new TGeoBBox(subsect_Fe_name, Zdc_Fe_X_size, Zdc_Fe_Y_size, subsect_Zdc_Fe_Z_size);
    TGeoVolume *subsect_FeV = new TGeoVolume(subsect_Fe_name, subsect_FeS);
    //TGeoMedium *Fe_medium = pMedIron; //set medium
    if(Fe_medium) subsect_FeV->SetMedium(Fe_medium);
    else Fatal("Main", "Invalid medium for subsect_Fe!");

    TGeoShape *subsect_PbS = new TGeoBBox(subsect_Pb_name, Zdc_Pb_X_size, Zdc_Pb_Y_size, subsect_Zdc_Pb_Z_size);
    TGeoVolume *subsect_PbV = new TGeoVolume(subsect_Pb_name, subsect_PbS);
    //TGeoMedium *Pb_medium = pMedLead; //set medium
    if(Pb_medium) subsect_PbV->SetMedium(Pb_medium);
    else Fatal("Main", "Invalid medium for subsect_Pb!");
    ...
    */
    ...
    mother_volume->AddNode(subsect_FeV,1,new TGeoTranslation(0, 0, -(Zdc_module_Z_size-subsect_Zdc_Fe_Z_size)));
    mother_volume->AddNode(FeV,1,new TGeoTranslation(0, 0, (Zdc_module_Z_size-Zdc_Fe_Z_size)));
    cout <<"subsect_Fe " << -(Zdc_module_Z_size-subsect_Zdc_Fe_Z_size) <<endl; 
    cout <<"Fe " <<(Zdc_module_Z_size-Zdc_Fe_Z_size) <<endl; 
    ...
    mother_volume->AddNode(TyvecV, 1, new TGeoTranslation(0,0, -(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*subsect_Zdc_Pb_Z_size-Zdc_Tyvec_Z_size)));
    cout <<"tyvec 1 " <<-(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*subsect_Zdc_Pb_Z_size-Zdc_Tyvec_Z_size) <<endl;
    ...
    //Pb
    //mother_volume->AddNode(PbV, 7, new TGeoTranslation(0, 0, -(Zdc_module_Z_size-2.*Zdc_Fe_Z_size-2.*Zdc_Tyvec_Z_size-2.*5.*(Zdc_Pb_Z_size+Zdc_Tyvec_Z_size)-Zdc_Pb_Z_size)));
    //cout <<"Pb 7 " <<-(Zdc_module_Z_size-2.*Zdc_Fe_Z_size-2.*Zdc_Tyvec_Z_size-2.*5.*(Zdc_Pb_Z_size+Zdc_Tyvec_Z_size)-Zdc_Pb_Z_size) <<endl;

    //subsect_Pb
    //mother_volume->AddNode(subsect_PbV, 1, new TGeoTranslation(0, 0, -(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-subsect_Zdc_Pb_Z_size)));
    //cout <<"subsect_Pb 1 " <<-(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-subsect_Zdc_Pb_Z_size) <<endl;
    ...
    //subsect Tyvec slices
    Double_t itInit = -(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*subsect_Zdc_Pb_Z_size-Zdc_Tyvec_Z_size); 
    Double_t itCur=0, itDelta=0;
    itDelta=2.*(subsect_Zdc_Pb_Z_size+Zdc_Tyvec_Z_size);
    for(Int_t it=1+1; it<subsect_Zdc_NPb_in_module+1; it++) {//12 slices (1-12), the first slice is already inserted
      itCur =itInit + (it-1)*itDelta;
      mother_volume->AddNode(TyvecV,it,new TGeoTranslation(0,0,itCur));
      cout <<"subsect it,itCur Ty " <<it <<" " <<itCur <<endl;
    }

    //Tyvec slices
    itInit = -(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*12.*(subsect_Zdc_Pb_Z_size+Zdc_Tyvec_Z_size)-2.*Zdc_Pb_Z_size-Zdc_Tyvec_Z_size); 
    itCur=0; itDelta=2.*(Zdc_Pb_Z_size+Zdc_Tyvec_Z_size);
    for(Int_t it=subsect_Zdc_NPb_in_module+1; it<Zdc_NTyvec_in_module+1; it++) {//slices (13-48)
      itCur =itInit + (it-(subsect_Zdc_NPb_in_module+1))*itDelta;
      mother_volume->AddNode(TyvecV,it,new TGeoTranslation(0,0,itCur));
      cout <<"it,itCur Ty " <<it <<" " <<itCur <<endl;
    }
    ...
    //subsect Pb slices    
    itInit=-(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-subsect_Zdc_Pb_Z_size); 
    itCur=0; itDelta=2.*(subsect_Zdc_Pb_Z_size+Zdc_Tyvec_Z_size);
    for(Int_t it=1; it<subsect_Zdc_NPb_in_module+1; it++) {//12 slices (1-12)
      itCur =itInit + (it-1)*itDelta;
      mother_volume->AddNode(subsect_PbV,it,new TGeoTranslation(0,0,itCur));
      cout <<"subsect it,itCur Pb " <<it <<" " <<itCur <<endl;
    }

    //Pb slices    
    itInit=-(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*12.*(subsect_Zdc_Pb_Z_size+Zdc_Tyvec_Z_size)-Zdc_Pb_Z_size); 
    itCur=0; itDelta=2.*(Zdc_Pb_Z_size+Zdc_Tyvec_Z_size);
    for(Int_t it=subsect_Zdc_NPb_in_module+1; it<Zdc_NPb_in_module+1; it++) {//48 slices (13-48)
      itCur =itInit + (it-(subsect_Zdc_NPb_in_module+1))*itDelta;
      mother_volume->AddNode(PbV,it,new TGeoTranslation(0,0,itCur));
      cout <<"it,itCur Pb " <<it <<" " <<itCur <<endl;
    }
    ...
    //subsects for module with hole 
    TString subsect_FeH_name = "subsect_FeH";
    TString subsect_PbH_name = "subsect_PbH";

    TString subsect_FeHTube_name = "subsect_FeHTube";
    TString subsect_PbHTube_name = "subsect_PbHTube";
    ...
    TGeoShape *subsect_FeHS = new TGeoBBox(subsect_FeH_name, Zdc_Fe_X_size, Zdc_Fe_Y_size, subsect_Zdc_Fe_Z_size);
    TGeoVolume *subsect_FeHV = new TGeoVolume(subsect_FeH_name, subsect_FeHS);
    //TGeoMedium *FeH_medium = pMedIron; //set medium
    if(FeH_medium) subsect_FeHV->SetMedium(FeH_medium);
    else Fatal("Main", "Invalid medium for subsect_FeH!");
    ...
    TGeoShape *subsect_PbHS = new TGeoBBox(subsect_PbH_name, Zdc_Pb_X_size, Zdc_Pb_Y_size, subsect_Zdc_Pb_Z_size);
    TGeoVolume *subsect_PbHV = new TGeoVolume(subsect_PbH_name, PbHS);
    //TGeoMedium *PbH_medium = pMedLead; //set medium
    if(PbH_medium) subsect_PbHV->SetMedium(PbH_medium);
    else Fatal("Main", "Invalid medium for subsect_PbH!");
    ...
    TGeoTube *subsect_FeHTubeS = new TGeoTube(subsect_FeHTube_name, Zdc_Hole_inner_radius, Zdc_Hole_outer_radius, subsect_Zdc_Fe_Z_size);
    TGeoVolume *subsect_FeHTubeV = new TGeoVolume(subsect_FeHTube_name, subsect_FeHTubeS);
    subsect_FeHTubeV->SetMedium(pMedVacuum);
    ...
    TGeoTube *subsect_PbHTubeS = new TGeoTube(subsect_PbHTube_name, Zdc_Hole_inner_radius, Zdc_Hole_outer_radius, subsect_Zdc_Pb_Z_size);
    TGeoVolume *subsect_PbHTubeV = new TGeoVolume(subsect_PbHTube_name, subsect_PbHTubeS);
    subsect_PbHTubeV->SetMedium(pMedVacuum);
    ...
    mother_volume_hole->AddNode(subsect_FeHV,1,new TGeoTranslation(0, 0, -(Zdc_module_Z_size-subsect_Zdc_Fe_Z_size)));
    //FeHV->AddNode(slot_FeV,1,new TGeoTranslation(Zdc_Fe_X_size-Zdc_VRFM_X_size, Zdc_Y_slot_position, 0));
    subsect_FeHV->AddNode(subsect_FeHTubeV,1,new TGeoTranslation(0., 0., 0.));
    ...
    cout <<"subsect_Fe hole " << -(Zdc_module_Z_size-subsect_Zdc_Fe_Z_size) <<endl; 
    cout <<"Fe hole " <<(Zdc_module_Z_size-Zdc_Fe_Z_size) <<endl; 
    ...
    subsect_PbHV->AddNode(subsect_PbHTubeV,1,new TGeoTranslation(0., 0., 0.));


    //insert Tyvec into module with hole
    mother_volume_hole->AddNode(TyvecHV, 1, new TGeoTranslation(0,0, -(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*subsect_Zdc_Pb_Z_size-Zdc_Tyvec_Z_size)));
    cout <<"tyvec 1 hole " <<-(Zdc_module_Z_size-2.*subsect_Zdc_Fe_Z_size-2.*subsect_Zdc_Pb_Z_size-Zdc_Tyvec_Z_size) <<endl;
    ...
    TyvecHV->AddNode(TyvecHTubeV,1,new TGeoTranslation(0., 0., -(Zdc_Tyvec_Z_size-Zdc_Hole_Tyvec_Z_size)));
    TyvecHV->AddNode(TyvecHTubeV,2,new TGeoTranslation(0., 0., (Zdc_Tyvec_Z_size-Zdc_Hole_Tyvec_Z_size)));
    ...
    for(Int_t it=1+1; it<subsect_Zdc_NPb_in_module+1; it++) {//12 slices (1-12), the first slice is already inserted
      itCur =itInit + (it-1)*itDelta;
      mother_volume_hole->AddNode(TyvecHV,it,new TGeoTranslation(0,0,itCur));
      cout <<"subsect it,itCur Ty hole" <<it <<" " <<itCur <<endl;
    }
    ...
    for(Int_t it=subsect_Zdc_NPb_in_module+1; it<Zdc_NTyvec_in_module+1; it++) {//slices (13-48)
      itCur =itInit + (it-(subsect_Zdc_NPb_in_module+1))*itDelta;
      mother_volume_hole->AddNode(TyvecHV,it,new TGeoTranslation(0,0,itCur));
      cout <<"it,itCur Ty hole " <<it <<" " <<itCur <<endl;
    }
    ...
    for(Int_t it=1; it<subsect_Zdc_NPb_in_module+1; it++) {//12 slices (1-12)
      itCur =itInit + (it-1)*itDelta;
      mother_volume_hole->AddNode(subsect_PbHV,it,new TGeoTranslation(0,0,itCur));
      cout <<"subsect it,itCur Pb hole " <<it <<" " <<itCur <<endl;
    }
    ...
    for(Int_t it=subsect_Zdc_NPb_in_module+1; it<Zdc_NPb_in_module+1; it++) {//48 slices (13-48)
      itCur =itInit + (it-(subsect_Zdc_NPb_in_module+1))*itDelta;
      mother_volume_hole->AddNode(PbHV,it,new TGeoTranslation(0,0,itCur));
      cout <<"it,itCur Pb hole " <<it <<" " <<itCur <<endl;
    }
