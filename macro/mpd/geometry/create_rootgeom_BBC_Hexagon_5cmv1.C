...
Int_t GetLayerIDOffset (TString side = "A", Int_t nRing = 0 ) {
    ...
    if( side.CompareTo("A") == 0 ) {
        ...
        if( nRing == 2  ) offSet = 12; //6;
        if( nRing == 3  ) offSet = 30;
        if( nRing == 4  ) offSet = 54;
        if( nRing == 5  ) offSet = 84;
        if( nRing == 6  ) offSet = 120;
        //if( nRing == 7  ) offSet = 126;
        
    } else if ( side.CompareTo("C") == 0) {
     
        if( nRing == 1  ) offSet = 162 ;
        if( nRing == 2  ) offSet = 174 ;// + 12;
        if( nRing == 3  ) offSet = 192 ;//+ 18;
        if( nRing == 4  ) offSet = 216 ;//+ 24;
        if( nRing == 5  ) offSet = 246 ;//+ 42;
        if( nRing == 6  ) offSet = 282 ;//+ 34;
...
void create_rootgeom_BBC_Hexagon_5cmv1(TString mediumBMD="NDetScin")
{
    ...
    const TString geoDetectorName = "bbc_hex_5cm";
  ...
  // BMDPlus
    ...
    hex->DefineSection(0,-1*cm, 0, 2.5*cm);//1.611*cm);
    hex->DefineSection(1, 1*cm, 0, 2.5*cm);//1.611*cm);
   ...
   for(Int_t i=0; i<ncells; i++) {
      ...
      if( i != 6 && i != 7 && i != 8 ) {
        ...
        TString cellAIdName = Form("Hexagon5cmA%d_%d",nring,cellId);
        TString cellCIdName = Form("Hexagon5cmC%d_%d",nring,cellId);
   ...
   for (Int_t i=0; i<ncells; i++) {
        ...
        TString cellARIdName = Form("Hexagon5cmA%d_%d",nring,cellIdR);
        TString cellALIdName = Form("Hexagon5cmA%d_%d",nring,cellIdL);
        TString cellCRIdName = Form("Hexagon5cmC%d_%d",nring,cellIdR);
        TString cellCLIdName = Form("Hexagon5cmC%d_%d",nring,cellIdL);
   ...
   for (Int_t i=0; i<ncells; i++) {
    ...
    Int_t nrows = 1;
    Int_t i=0;
    Double_t shiftx = -2.1650635;//-4.3305;
    Double_t xrow = 5*(2*i+1)*dxrow + shiftx;
    
    Double_t ytooth2  = - ((4.66666*dshifty)*6)/2;  //.0;  
    Double_t yrow = 0 + ytooth2; //0 + ytooth2;  
