//**************************************************************************************
// MPDROOT TOF version 8.3
// Added a cable and chip volumes. 
// commented volumes: vOuterTube, vInnerTube, vTrapez, vTooth --->(overlapping emcChamber)
// 27.10.2020
//**************************************************************************************
...
//----------------------------------------------------------------------------------------------------------------------
void    CreateCables(TGeoVolumeAssembly *vSector, TGeoMedium *medAl, TGeoMedium *medG10, TGeoMedium *medCables);
void    InstallDetectorsAndCreateMounts(TGeoVolumeAssembly *vSector, TGeoVolume *vDetBox, TGeoMedium *medAl, double Zstep, double Z0, double SecBoxZshift, double SecBox2Zshift);
void    CreateWheel(TGeoVolumeAssembly *vSector, TGeoMedium *medAl);
void    CreatePlates(TGeoVolumeAssembly *vSector, TGeoMedium *medAl, double SecBoxZshift, double SecBox2Zshift);
void    CreatePowerFrame(TGeoVolumeAssembly *top, TGeoMedium *medAl);
//----------------------------------------------------------------------------------------------------------------------
void MakeMPDROOT_TOFv8_3()
{
    ...
    const TString geoFileName = geoPath + "/geometry/tof_v8_3.root";
    ...
    auto motherTop = new TGeoVolumeAssembly("TOP"); 

    gGeoManager->SetTopVolume(motherTop);
    gGeoManager->SetTopVisible();

    auto top = new TGeoVolumeAssembly("tof1"); // tof TOP VOLUME
    // ------------------------------------------------------ Create sector volume ----------------------------------------------
    const size_t Nsectors =  14;
    auto vSector = new TGeoVolumeAssembly("tof1Sector");
    // ----------------------------------------------------------- ASSEMBLE sector ----------------------------------------------
    ...
    CreateCables(vSector, medAl, medG10, medAl);
    
    CreatePlates(vSector, medAl, SecBoxZshift, SecBox2Zshift);
 
    //------------- Create detector box ----------------
    auto vDetBox = new TGeoVolumeAssembly("tof1DetectorBox");

    // ----------------------------------------------------------- ASSEMBLE detector ----------------------------------------------
    const double Ysize = 2.388, Ygap = 0.380; // [cm]
    const double solPlateX = 64.5,   solPlateY = 0.06,        solPlateZ = 30.0;   
    const double stripGasX = 65.,    stripGasY = 0.1,         stripGasZ = 1.25;
    const double stripCuX = 65.,     stripCuY = 0.0035,       stripCuZ = 1.;   
    const double detG10ThickX = 65., detG10ThickY = 0.07325,  detG10ThickZ = 33.0;   
    const double detG10ThinX = 65.,  detG10ThinY = 0.1,       detG10ThinZ = 33.0; 
    const double detGlassX = 64.5,   detGlassY = 0.196,       detGlassZ = 30.0;
    
    cerr<<"\n det. total size="<<solPlateY + Ygap + solPlateY + detG10ThinY +
        detGlassY + stripGasY + detG10ThickY + stripCuY + detG10ThickY + 
        detGlassY + stripGasY + detG10ThickY + stripCuY + detG10ThickY +   
        detGlassY + stripGasY +
        detG10ThinY + solPlateY + Ygap + solPlateY<<" MUSTBE= "<<Ysize<<endl; 
    
    //--------------------- SOLIDS (HONEYCOMB) -----------------------
    auto sSolPlate = new TGeoBBox(solPlateX/2., solPlateY/2., solPlateZ/2.);
    auto vSolPlate = new TGeoVolume("tof1SolPlateV", sSolPlate, medG10);                    vSolPlate->SetLineColor(kBlack);

    // Add solids
    const double Yplate12 = Ysize/2.- solPlateY/2.;
    const double Yplate34 = Yplate12 - Ygap - solPlateY;     
    vDetBox->AddNode(vSolPlate, 1, new TGeoTranslation("tdetPlate",  0., +Yplate12, 0.));
    vDetBox->AddNode(vSolPlate, 2, new TGeoTranslation("tdetPlate",  0., -Yplate12, 0.));
    vDetBox->AddNode(vSolPlate, 3, new TGeoTranslation("tdetPlate",  0., +Yplate34, 0.)); 
    vDetBox->AddNode(vSolPlate, 4, new TGeoTranslation("tdetPlate",  0., -Yplate34, 0.));

    //----------------------- G10 THIN (1&2) -------------------------
    auto sDetG10Thin = new TGeoBBox(detG10ThinX/2., detG10ThinY/2., detG10ThinZ/2.); 
    auto vDetG10Thin = new TGeoVolume("tof1DetG10ThinV", sDetG10Thin, medG10);              vDetG10Thin->SetLineColor(kGreen);

    // Add Glass thin plates
    const double YG10Thin = Yplate34 - solPlateY/2. - detG10ThinY/2.;
    vDetBox->AddNode(vDetG10Thin, 1, new TGeoTranslation("tdetPlate",  0., +YG10Thin, 0.));
    vDetBox->AddNode(vDetG10Thin, 2, new TGeoTranslation("tdetPlate",  0., -YG10Thin, 0.)); 
    
    //--------------------- GLASS (1&2) -----------------------
    auto sDetGlass = new TGeoBBox(detGlassX/2., detGlassY/2., detGlassZ/2.);   
    auto vDetGlass = new TGeoVolume("tof1DetGlassV", sDetGlass, medGlass);                  vDetGlass->SetLineColor(kBlue);

    // Add Glass
    const double Yglass1 = +YG10Thin - detG10ThinY/2. - detGlassY/2.;
    const double Yglass3 = -YG10Thin + detG10ThinY/2. + stripGasY + detGlassY/2.;  
    const double Yglass2 = Yglass3  + detGlassY/2.+ (detG10ThickY + stripCuY + detG10ThickY) + stripGasY + detGlassY/2.;    
    vDetBox->AddNode(vDetGlass, 1, new TGeoTranslation("tdetGlass",  0., Yglass1, 0.));
    vDetBox->AddNode(vDetGlass, 2, new TGeoTranslation("tdetGlass",  0., Yglass2, 0.));
    vDetBox->AddNode(vDetGlass, 3, new TGeoTranslation("tdetGlass",  0., Yglass3, 0.));

    //----------------------- G10 THICK (1&2) -------------------------
    auto sDetG10Thick = new TGeoBBox(detG10ThickX/2., detG10ThickY/2., detG10ThickZ/2.);  
    auto vDetG10Thick = new TGeoVolume("tof1DetG10ThickV", sDetG10Thick, medG10);           vDetG10Thick->SetLineColor(kGreen);

    // Add Glass thin plates
    const double YG10Thick2 = Yglass2 + detGlassY/2. + detG10ThickY/2.;
    const double YG10Thick1 = YG10Thick2 +  detG10ThickY/2. +  stripCuY + detG10ThickY/2.;
    const double YG10Thick4 = Yglass3 + detGlassY/2. + detG10ThickY/2.;
    const double YG10Thick3 = YG10Thick4 +  detG10ThickY/2. +  stripCuY + detG10ThickY/2.;   
    vDetBox->AddNode(vDetG10Thick, 1, new TGeoTranslation("tdetPlate",  0., YG10Thick1, 0.));
    vDetBox->AddNode(vDetG10Thick, 3, new TGeoTranslation("tdetPlate",  0., YG10Thick2, 0.));
    vDetBox->AddNode(vDetG10Thick, 2, new TGeoTranslation("tdetPlate",  0., YG10Thick3, 0.));
    vDetBox->AddNode(vDetG10Thick, 4, new TGeoTranslation("tdetPlate",  0., YG10Thick4, 0.));

    //------------------------------ STRIPS ----------------------
    auto sStripGas = new TGeoBBox(stripGasX/2., stripGasY/2., stripGasZ/2.);  
    auto vStripGas = new TGeoVolume("tof1StripActiveGasV", sStripGas, medGas);              vStripGas->SetLineColor(kYellow);

    // Add strips
    const int NStrips = 24; // 24 strips group
    const double StripStep = 1.25; // [cm]
    const double Ygas1 =  Yglass1 - detGlassY/2. - stripGasY/2.;
    const double Ygas2 =  Yglass2 - detGlassY/2. - stripGasY/2.;
    const double Ygas3 =  Yglass3 - detGlassY/2. - stripGasY/2.;
    ...
    for(int i = 1; i <= NStrips; i++) 
    {
        ...
        vDetBox->AddNode(vStripGas, 2*NStrips + i, new TGeoTranslation("",  0., Ygas3, Z)); // [49,72]
    }
    
    //------------------------ CU STRIPS IN G10 THICK ---------------------
    auto sStripCu = new TGeoBBox(stripCuX/2., stripCuY/2., stripCuZ/2.); 
    ...
    // ----------------------------------------------------------- ASSEMBLE sector ---------------------------------------------------------
    double Zstep = 28.15, Z0 = -126.6185;
    
    InstallDetectorsAndCreateMounts(vSector, vDetBox, medAl, Zstep, Z0, SecBoxZshift, SecBox2Zshift);
    
    CreateWheel(vSector, medAl);
    ...
    CreatePowerFrame(top, medAl); 
    
    // ------------------------------------------------------Install Tof to mother volume -------------------------------------------------------
    motherTop->AddNode(top, 1, new TGeoTranslation("tw",  0.,  0., 0.)); 
    
    cerr<<endl<<endl;
    // --------------------------------------------------------------------------------   Finish   -------------------------------------------->>>
    // ------------------------------------------------- Check overlaps
    gGeoManager->CloseGeometry();   
 //   gGeoManager->CheckOverlaps(0.0001,  "s100000000000");
...
void CreateCables(TGeoVolumeAssembly *vSector, TGeoMedium *medAl, TGeoMedium *medG10, TGeoMedium *medCables)
{
    ...
    TGeoCompositeShape *sAllTrimBoxes1 = new TGeoCompositeShape("sAllTrimBoxes1",  "sTrimBox1:tTrimBox1_1+sTrimBox1:tTrimBox1_2");
    ...
    TGeoCompositeShape *sAllTrimBoxes2 = new TGeoCompositeShape("sAllTrimBoxes2",   "sTrimBox2:tTrimBox2_1+sTrimBox2:tTrimBox2_2");
    ...
    TGeoCompositeShape *sFourHoles = new TGeoCompositeShape("sFourHoles",    "sTrimTube:tTrimTube1+sTrimTube:tTrimTube2+sTrimTube:tTrimTube3+sTrimTube:tTrimTube4");
    ...
    TGeoCompositeShape *sRawOfHoles = new TGeoCompositeShape("sRawOfHoles",    "sFourHoles:tFourHoles1+sFourHoles:tFourHoles2+sFourHoles:tFourHoles3");

    // Assemble Cabe Holders
    TGeoCompositeShape *sCableHolder1 = new TGeoCompositeShape("sCableHolder1",    "sCableHolderBody-(sAllTrimBoxes1+sRawOfHoles)");
    TGeoCompositeShape *sCableHolder2 = new TGeoCompositeShape("sCableHolder2",    "sCableHolderBody-(sAllTrimBoxes2+sRawOfHoles)");
    ...
    vCableHolder2->SetLineColor(kBlue);

    // Adding Cable Holders
    for(int i = 0; i < 9; i++)
    {
        if(i < 5)       vSector->AddNode(vCableHolder1, i+1, new TGeoTranslation(0, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, 28.6 + 28 * i + 0.7/2) );
        else            vSector->AddNode(vCableHolder2, i+1, new TGeoTranslation(0, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, 28.6 + 28 * i + 0.7/2) );
        
    }

    for(int i = 0; i < 9; i++)
    {
        if(i < 5)       vSector->AddNode(vCableHolder1, i+10, new TGeoTranslation(0, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, -29.6 - 29.0 * i - 0.7/2) );
        else            vSector->AddNode(vCableHolder2, i+10, new TGeoTranslation(0, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, -29.6 - 29.0 * i - 0.7/2) );
    }

    //------------------ Chip ----------------------------------------------------------------------------
    const float ChipX = 8.9, ChipY = 0.2, ChipZ = 19.6;
    TGeoBBox *sChip = new TGeoBBox("sChip", ChipX/2, ChipY/2, ChipZ/2);
    TGeoVolume *vChip = new TGeoVolume("vChip", sChip, medG10);
    vChip->SetLineColor(kRed);

    // ----------- Adding chips ---------------------
    int g = 1;
    for(int i = 0; i < 9; i++)
    {
        vSector->AddNode(vChip, g, new TGeoTranslation( 24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, 28.6 + 28 * i + 0.7/2+14.) );
        g++;
        vSector->AddNode(vChip, g, new TGeoTranslation(-24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, 28.6 + 28 * i + 0.7/2+14.) );
        g++;
    }
    ...
    for(int i = 0; i < 9; i++)
    {
        vSector->AddNode(vChip, g, new TGeoTranslation( 24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, -29.6 - 29.0 * i - 0.7/2 - 14.) );
        g++;
        vSector->AddNode(vChip, g, new TGeoTranslation(-24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, -29.6 - 29.0 * i - 0.7/2 - 14.) );
        g++;
    }
    
    vSector->AddNode(vChip, g, new TGeoTranslation( 24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2,  28.6 + 0.7/2 - 14.5) );
    vSector->AddNode(vChip, g, new TGeoTranslation(-24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2,  28.6 + 0.7/2 - 14.5) );
    vSector->AddNode(vChip, g, new TGeoTranslation( 24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, -29.6 - 0.7/2 + 14.) );
    vSector->AddNode(vChip, g, new TGeoTranslation(-24.85, 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2, -29.6 - 0.7/2 + 14.) );

    //Cables---------------------------------------------------------------------------------------------------------------------
    const float CableShiftX = 12.35, CableShiftY = 1.85 + 0.5 - 0.035 + 0.07/2 + 4.5/2 - 0.24;
    const float CableShiftRightZ =  31.0 + 0.2 + 107.65 +281.2/2 + 0.3 - 0.3/2, CableShiftLeftZ = -31.0 - 1.95 - 0.2 - 221.4/2 - 291.2/2 - 0.3 + 0.3/2;


    //Right Sector Cables
    TGeoTube *sCableStrait1 = new TGeoTube("sCableStrait1", 0, 1.3/2, (26.68 + 28.0*8 + 0.3)/2);
    TGeoTranslation *tCableStrait1 = new TGeoTranslation("tCableStrait1", CableShiftX + 1.3*2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*8 + 0.3)/2);
    tCableStrait1->RegisterYourself();
    
    TGeoTube *sCableStrait2 = new TGeoTube("sCableStrait2", 0, 1.3/2, (26.68 + 28.0*7 + 0.3)/2);
    TGeoTranslation *tCableStrait2 = new TGeoTranslation("tCableStrait2", CableShiftX + 1.3, CableShiftY, CableShiftRightZ-(26.68 + 28.0*7 + 0.3)/2);
    tCableStrait2->RegisterYourself();
    
    TGeoTube *sCableStrait3 = new TGeoTube("sCableStrait3", 0, 1.3/2, (26.68 + 28.0*6 + 0.3)/2);
    TGeoTranslation *tCableStrait3 = new TGeoTranslation("tCableStrait3", CableShiftX, CableShiftY, CableShiftRightZ-(26.68 + 28.0*6 + 0.3)/2);
    tCableStrait3->RegisterYourself();
    
    TGeoTube *sCableStrait4 = new TGeoTube("sCableStrait4", 0, 1.3/2, (26.68 + 28.0*5 + 0.3)/2);
    TGeoTranslation *tCableStrait4 = new TGeoTranslation("tCableStrait4", CableShiftX - 1.3, CableShiftY, CableShiftRightZ-(26.68 + 28.0*5 + 0.3)/2);
    tCableStrait4->RegisterYourself();
    
    TGeoTube *sCableStrait5 = new TGeoTube("sCableStrait5", 0, 1.3/2, (26.68 + 28.0*4 + 0.3)/2);
    TGeoTranslation *tCableStrait5 = new TGeoTranslation("tCableStrait5", CableShiftX - 1.3*2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*4 + 0.3)/2);
    tCableStrait5->RegisterYourself();
    
    TGeoTube *sCableStrait6 = new TGeoTube("sCableStrait6", 0, 1.3/2, (26.68 + 28.0*3 + 0.3)/2);
    TGeoTranslation *tCableStrait6 = new TGeoTranslation("tCableStrait6", CableShiftX + 1.3*2, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 28.0*3 + 0.3)/2);
    tCableStrait6->RegisterYourself();
    
    TGeoTube *sCableStrait7 = new TGeoTube("sCableStrait7", 0, 1.3/2, (26.68 + 28.0*2 + 0.3)/2);
    TGeoTranslation *tCableStrait7 = new TGeoTranslation("tCableStrait7", CableShiftX + 1.3, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 28.0*2 + 0.3)/2);
    tCableStrait7->RegisterYourself();
    
    TGeoTube *sCableStrait8 = new TGeoTube("sCableStrait8", 0, 1.3/2, (26.68 + 28.0 + 0.3)/2);
    TGeoTranslation *tCableStrait8 = new TGeoTranslation("tCableStrait8", CableShiftX, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 28.0 + 0.3)/2);
    tCableStrait8->RegisterYourself();
    
    TGeoTube *sCableStrait9 = new TGeoTube("sCableStrait9", 0, 1.3/2, (26.68 + 0.3)/2);
    TGeoTranslation *tCableStrait9 = new TGeoTranslation("tCableStrait9", CableShiftX - 1.3, CableShiftY + 1.3, CableShiftRightZ - (26.68 + 0.3)/2);
    tCableStrait9->RegisterYourself();

    TGeoTube *sCableStrait10 = new TGeoTube("sCableStrait10", 0, 1.3/2, 0.3/2);
    TGeoTranslation *tCableStrait10 = new TGeoTranslation("tCableStrait10", CableShiftX - 1.3*2, CableShiftY + 1.3, CableShiftRightZ - 0.3/2);
    tCableStrait10->RegisterYourself();

    TGeoCompositeShape *sCableStraitCombR1 = new TGeoCompositeShape("sCableStraitCombR1",
    "sCableStrait1:tCableStrait1+sCableStrait2:tCableStrait2+sCableStrait3:tCableStrait3+sCableStrait4:tCableStrait4+sCableStrait5:tCableStrait5");
    TGeoCompositeShape *sCableStraitCombR2 = new TGeoCompositeShape("sCableStraitCombR2",
    "sCableStrait6:tCableStrait6+sCableStrait7:tCableStrait7+sCableStrait8:tCableStrait8+sCableStrait9:tCableStrait9+sCableStrait10:tCableStrait10");

    TGeoRotation *rCableStraitCombR = new TGeoRotation("rCableStraitCombR", 0, 0, 180);
    TGeoCombiTrans *cCableStraitCombR1 = new TGeoCombiTrans("cCableStraitCombR1", 0, CableShiftY*2, 0, rCableStraitCombR);
    TGeoCombiTrans *cCableStraitCombR2 = new TGeoCombiTrans("cCableStraitCombR2", 0, CableShiftY*2+1.3*2, 0, rCableStraitCombR);

    cCableStraitCombR1->RegisterYourself();
    cCableStraitCombR2->RegisterYourself();


    //Round Cable Right---------------------------------------------------------------------------
    TGeoTorus *sCableRound = new TGeoTorus("sCableRound", 14.0, 0, 1.3/2, 0, 90);
    TGeoRotation *rCableRoundR = new TGeoRotation("rCableRoundR", 0, -90, 0);

    TGeoCombiTrans *cCableRoundR1 = new TGeoCombiTrans("cCableRoundR1", CableShiftX + 1.3*2 - 14.0, CableShiftY, CableShiftRightZ-(26.68 + 28.0*8 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR2 = new TGeoCombiTrans("cCableRoundR2", CableShiftX + 1.3 - 14.0, CableShiftY, CableShiftRightZ-(26.68 + 28.0*7 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR3 = new TGeoCombiTrans("cCableRoundR3", CableShiftX - 14.0, CableShiftY, CableShiftRightZ-(26.68 + 28.0*6 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR4 = new TGeoCombiTrans("cCableRoundR4", CableShiftX - 1.3 - 14.0, CableShiftY, CableShiftRightZ-(26.68 + 28.0*5 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR5 = new TGeoCombiTrans("cCableRoundR5", CableShiftX - 1.3*2 - 14.0, CableShiftY, CableShiftRightZ-(26.68 + 28.0*4 + 0.3), rCableRoundR);

    TGeoCombiTrans *cCableRoundR6 = new TGeoCombiTrans("cCableRoundR6", CableShiftX + 1.3*2 - 14.0, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 28.0*3 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR7 = new TGeoCombiTrans("cCableRoundR7", CableShiftX + 1.3 - 14.0, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 28.0*2 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR8 = new TGeoCombiTrans("cCableRoundR8", CableShiftX - 14.0, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 28.0*1 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR9 = new TGeoCombiTrans("cCableRoundR9", CableShiftX - 1.3 - 14.0, CableShiftY + 1.3, CableShiftRightZ-(26.68 + 0.3), rCableRoundR);
    TGeoCombiTrans *cCableRoundR10 = new TGeoCombiTrans("cCableRoundR10", CableShiftX - 1.3*2 - 14.0, CableShiftY + 1.3, CableShiftRightZ-(0.3), rCableRoundR);

    cCableRoundR1->RegisterYourself();
    cCableRoundR2->RegisterYourself();
    cCableRoundR3->RegisterYourself();
    cCableRoundR4->RegisterYourself();
    cCableRoundR5->RegisterYourself();
    cCableRoundR6->RegisterYourself();
    cCableRoundR7->RegisterYourself();
    cCableRoundR8->RegisterYourself();
    cCableRoundR9->RegisterYourself();
    cCableRoundR10->RegisterYourself();

    TGeoCompositeShape *sCableRoundCombR1 = new TGeoCompositeShape("sCableRoundCombR1",
    "sCableRound:cCableRoundR1+sCableRound:cCableRoundR2+sCableRound:cCableRoundR3+sCableRound:cCableRoundR4+sCableRound:cCableRoundR5");
    TGeoCompositeShape *sCableRoundCombR2 = new TGeoCompositeShape("sCableRoundCombR2",
    "sCableRound:cCableRoundR6+sCableRound:cCableRoundR7+sCableRound:cCableRoundR8+sCableRound:cCableRoundR9+sCableRound:cCableRoundR10");

    //Cable to chip right ------------------------------------------------------------------------
    TGeoTube *sCableToChip1 = new TGeoTube("sCableToChip1", 0, 1.3/2, (24.85 - 8.9/2 + CableShiftX + 1.3*2 - 14.0)/2);
    TGeoTube *sCableToChip2 = new TGeoTube("sCableToChip2", 0, 1.3/2, (24.85 - 8.9/2 + CableShiftX + 1.3*1 - 14.0)/2);
    TGeoTube *sCableToChip3 = new TGeoTube("sCableToChip3", 0, 1.3/2, (24.85 - 8.9/2 + CableShiftX + 1.3*0 - 14.0)/2);
    TGeoTube *sCableToChip4 = new TGeoTube("sCableToChip4", 0, 1.3/2, (24.85 - 8.9/2 + CableShiftX - 1.3*1 - 14.0)/2);
    TGeoTube *sCableToChip5 = new TGeoTube("sCableToChip5", 0, 1.3/2, (24.85 - 8.9/2 + CableShiftX - 1.3*2 - 14.0)/2);

    TGeoRotation *rCableToChip = new TGeoRotation("rCableToChip", 90, 90, 0);

    TGeoCombiTrans *cCableToChip1 = new TGeoCombiTrans("cCableToChip1", (CableShiftX + 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*8 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip2 = new TGeoCombiTrans("cCableToChip2", (CableShiftX + 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*7 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip3 = new TGeoCombiTrans("cCableToChip3", (CableShiftX + 1.3*0 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*6 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip4 = new TGeoCombiTrans("cCableToChip4", (CableShiftX - 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*5 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip5 = new TGeoCombiTrans("cCableToChip5", (CableShiftX - 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftRightZ-(26.68 + 28.0*4 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip6 = new TGeoCombiTrans("cCableToChip6", (CableShiftX + 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftRightZ-(26.68 + 28.0*3 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip7 = new TGeoCombiTrans("cCableToChip7", (CableShiftX + 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftRightZ-(26.68 + 28.0*2 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip8 = new TGeoCombiTrans("cCableToChip8", (CableShiftX + 1.3*0 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftRightZ-(26.68 + 28.0*1 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip9 = new TGeoCombiTrans("cCableToChip9", (CableShiftX - 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftRightZ-(26.68 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip10 = new TGeoCombiTrans("cCableToChip10", (CableShiftX - 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftRightZ-(0.3 + 14.0), rCableToChip);

    cCableToChip1->RegisterYourself();
    cCableToChip2->RegisterYourself();
    cCableToChip3->RegisterYourself();
    cCableToChip4->RegisterYourself();
    cCableToChip5->RegisterYourself();
    cCableToChip6->RegisterYourself();
    cCableToChip7->RegisterYourself();
    cCableToChip8->RegisterYourself();
    cCableToChip9->RegisterYourself();
    cCableToChip10->RegisterYourself();

    TGeoCompositeShape *sCableToChipCombR1 = new TGeoCompositeShape("sCableToChipCombR1",
    "sCableToChip1:cCableToChip1+sCableToChip2:cCableToChip2+sCableToChip3:cCableToChip3+sCableToChip4:cCableToChip4+sCableToChip5:cCableToChip5");
    TGeoCompositeShape *sCableToChipCombR2 = new TGeoCompositeShape("sCableToChipCombR2",
    "sCableToChip1:cCableToChip6+sCableToChip2:cCableToChip7+sCableToChip3:cCableToChip8+sCableToChip4:cCableToChip9+sCableToChip5:cCableToChip10");

    //------Combine R-------------
    TGeoCompositeShape *sCableCombR1 = new TGeoCompositeShape("sCableCombR1",
    "sCableStraitCombR1+sCableRoundCombR1+sCableToChipCombR1");
    TGeoCompositeShape *sCableCombR2 = new TGeoCompositeShape("sCableCombR2",
    "sCableStraitCombR2+sCableRoundCombR2+sCableToChipCombR2");

    //Left Sector Cables
    TGeoTube *sCableStrait11 = new TGeoTube("sCableStrait11", 0, 1.3/2, (27.65 + 29.0*8 + 0.3)/2);
    TGeoTranslation *tCableStrait11 = new TGeoTranslation("tCableStrait11", CableShiftX + 1.3*2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*8 + 0.3)/2);
    tCableStrait11->RegisterYourself();
    
    TGeoTube *sCableStrait12 = new TGeoTube("sCableStrait12", 0, 1.3/2, (27.65 + 29.0*7 + 0.3)/2);
    TGeoTranslation *tCableStrait12 = new TGeoTranslation("tCableStrait12", CableShiftX + 1.3, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*7 + 0.3)/2);
    tCableStrait12->RegisterYourself();
    
    TGeoTube *sCableStrait13 = new TGeoTube("sCableStrait13", 0, 1.3/2, (27.65 + 29.0*6 + 0.3)/2);
    TGeoTranslation *tCableStrait13 = new TGeoTranslation("tCableStrait13", CableShiftX, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*6 + 0.3)/2);
    tCableStrait13->RegisterYourself();
    
    TGeoTube *sCableStrait14 = new TGeoTube("sCableStrait14", 0, 1.3/2, (27.65 + 29.0*5 + 0.3)/2);
    TGeoTranslation *tCableStrait14 = new TGeoTranslation("tCableStrait14", CableShiftX - 1.3, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*5 + 0.3)/2);
    tCableStrait14->RegisterYourself();
    
    TGeoTube *sCableStrait15 = new TGeoTube("sCableStrait15", 0, 1.3/2, (27.65 + 29.0*4 + 0.3)/2);
    TGeoTranslation *tCableStrait15 = new TGeoTranslation("tCableStrait15", CableShiftX - 1.3*2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*4 + 0.3)/2);
    tCableStrait15->RegisterYourself();
    
    TGeoTube *sCableStrait16 = new TGeoTube("sCableStrait16", 0, 1.3/2, (27.65 + 29.0*3 + 0.3)/2);
    TGeoTranslation *tCableStrait16 = new TGeoTranslation("tCableStrait16", CableShiftX + 1.3*2, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 29.0*3 + 0.3)/2);
    tCableStrait16->RegisterYourself();
    
    TGeoTube *sCableStrait17 = new TGeoTube("sCableStrait17", 0, 1.3/2, (27.65 + 29.0*2 + 0.3)/2);
    TGeoTranslation *tCableStrait17 = new TGeoTranslation("tCableStrait17", CableShiftX + 1.3, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 29.0*2 + 0.3)/2);
    tCableStrait17->RegisterYourself();
    
    TGeoTube *sCableStrait18 = new TGeoTube("sCableStrait18", 0, 1.3/2, (27.65 + 29.0 + 0.3)/2);
    TGeoTranslation *tCableStrait18 = new TGeoTranslation("tCableStrait18", CableShiftX, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 29.0 + 0.3)/2);
    tCableStrait18->RegisterYourself();
    
    TGeoTube *sCableStrait19 = new TGeoTube("sCableStrait19", 0, 1.3/2, (27.65 + 0.3)/2);
    TGeoTranslation *tCableStrait19 = new TGeoTranslation("tCableStrait19", CableShiftX - 1.3, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 0.3)/2);
    tCableStrait19->RegisterYourself();

    TGeoTube *sCableStrait20 = new TGeoTube("sCableStrait20", 0, 1.3/2, 0.3/2);
    TGeoTranslation *tCableStrait20 = new TGeoTranslation("tCableStrait20", CableShiftX - 1.3*2, CableShiftY + 1.3, CableShiftLeftZ + 0.3/2);
    tCableStrait20->RegisterYourself();

    TGeoCompositeShape *sCableStraitCombL1 = new TGeoCompositeShape("sCableStraitCombL1",
    "sCableStrait11:tCableStrait11+sCableStrait12:tCableStrait12+sCableStrait13:tCableStrait13+sCableStrait14:tCableStrait14+sCableStrait15:tCableStrait15");
    TGeoCompositeShape *sCableStraitCombL2 = new TGeoCompositeShape("sCableStraitCombL2",
    "sCableStrait16:tCableStrait16+sCableStrait17:tCableStrait17+sCableStrait18:tCableStrait18+sCableStrait19:tCableStrait19+sCableStrait20:tCableStrait20");

    TGeoRotation *rCableStraitCombL = new TGeoRotation("rCableStraitCombL", 0, 0, 180);
    TGeoCombiTrans *cCableStraitCombL1 = new TGeoCombiTrans("cCableStraitCombL1", 0, CableShiftY*2, 0, rCableStraitCombL);
    TGeoCombiTrans *cCableStraitCombL2 = new TGeoCombiTrans("cCableStraitCombL2", 0, CableShiftY*2+1.3*2, 0, rCableStraitCombL);

    //Round Cable Left---------------------------------------------------------------------------
    TGeoRotation *rCableRoundL = new TGeoRotation("rCableRoundL", 0, 90, 0);

    TGeoCombiTrans *cCableRoundL1 = new TGeoCombiTrans("cCableRoundL1", CableShiftX + 1.3*2 - 14.0, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*8 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL2 = new TGeoCombiTrans("cCableRoundL2", CableShiftX + 1.3 - 14.0, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*7 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL3 = new TGeoCombiTrans("cCableRoundL3", CableShiftX - 14.0, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*6 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL4 = new TGeoCombiTrans("cCableRoundL4", CableShiftX - 1.3 - 14.0, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*5 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL5 = new TGeoCombiTrans("cCableRoundL5", CableShiftX - 1.3*2 - 14.0, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*4 + 0.3), rCableRoundL);

    TGeoCombiTrans *cCableRoundL6 = new TGeoCombiTrans("cCableRoundL6", CableShiftX + 1.3*2 - 14.0, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 29.0*3 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL7 = new TGeoCombiTrans("cCableRoundL7", CableShiftX + 1.3 - 14.0, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 29.0*2 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL8 = new TGeoCombiTrans("cCableRoundL8", CableShiftX - 14.0, CableShiftY + 1.3, CableShiftLeftZ+(27.65 + 29.0*1 + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL9 = new TGeoCombiTrans("cCableRoundL9", CableShiftX - 1.3 - 14.0, CableShiftY + 1.3, CableShiftLeftZ+(27.65  + 0.3), rCableRoundL);
    TGeoCombiTrans *cCableRoundL10 = new TGeoCombiTrans("cCableRoundL10", CableShiftX - 1.3*2 - 14.0, CableShiftY + 1.3, CableShiftLeftZ+(0.3), rCableRoundL);

    cCableRoundL1->RegisterYourself();
    cCableRoundL2->RegisterYourself();
    cCableRoundL3->RegisterYourself();
    cCableRoundL4->RegisterYourself();
    cCableRoundL5->RegisterYourself();
    cCableRoundL6->RegisterYourself();
    cCableRoundL7->RegisterYourself();
    cCableRoundL8->RegisterYourself();
    cCableRoundL9->RegisterYourself();
    cCableRoundL10->RegisterYourself();

    TGeoCompositeShape *sCableRoundCombL1 = new TGeoCompositeShape("sCableRoundCombL1",
    "sCableRound:cCableRoundL1+sCableRound:cCableRoundL2+sCableRound:cCableRoundL3+sCableRound:cCableRoundL4+sCableRound:cCableRoundL5");
    TGeoCompositeShape *sCableRoundCombL2 = new TGeoCompositeShape("sCableRoundCombL2",
    "sCableRound:cCableRoundL6+sCableRound:cCableRoundL7+sCableRound:cCableRoundL8+sCableRound:cCableRoundL9+sCableRound:cCableRoundL10");

    //Cable to chip left ------------------------------------------------------------------------
    TGeoCombiTrans *cCableToChip11 = new TGeoCombiTrans("cCableToChip11", (CableShiftX + 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*8 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip12 = new TGeoCombiTrans("cCableToChip12", (CableShiftX + 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*7 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip13 = new TGeoCombiTrans("cCableToChip13", (CableShiftX + 1.3*0 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*6 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip14 = new TGeoCombiTrans("cCableToChip14", (CableShiftX - 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*5 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip15 = new TGeoCombiTrans("cCableToChip15", (CableShiftX - 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY, CableShiftLeftZ+(27.65 + 29.0*4 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip16 = new TGeoCombiTrans("cCableToChip16", (CableShiftX + 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftLeftZ+(27.65 + 29.0*3 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip17 = new TGeoCombiTrans("cCableToChip17", (CableShiftX + 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftLeftZ+(27.65 + 29.0*2 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip18 = new TGeoCombiTrans("cCableToChip18", (CableShiftX + 1.3*0 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftLeftZ+(27.65 + 29.0*1 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip19 = new TGeoCombiTrans("cCableToChip19", (CableShiftX - 1.3*1 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftLeftZ+(27.65 + 0.3 + 14.0), rCableToChip);
    TGeoCombiTrans *cCableToChip20 = new TGeoCombiTrans("cCableToChip20", (CableShiftX - 1.3*2 - 14.0 - 24.85 + 8.9/2)/2, CableShiftY+1.3, CableShiftLeftZ+(0.3 + 14.0), rCableToChip);

    cCableToChip11->RegisterYourself();
    cCableToChip12->RegisterYourself();
    cCableToChip13->RegisterYourself();
    cCableToChip14->RegisterYourself();
    cCableToChip15->RegisterYourself();
    cCableToChip16->RegisterYourself();
    cCableToChip17->RegisterYourself();
    cCableToChip18->RegisterYourself();
    cCableToChip19->RegisterYourself();
    cCableToChip20->RegisterYourself();

    TGeoCompositeShape *sCableToChipCombL1 = new TGeoCompositeShape("sCableToChipCombL1",
    "sCableToChip1:cCableToChip11+sCableToChip2:cCableToChip12+sCableToChip3:cCableToChip13+sCableToChip4:cCableToChip14+sCableToChip5:cCableToChip15");
    TGeoCompositeShape *sCableToChipCombL2 = new TGeoCompositeShape("sCableToChipCombL2",
    "sCableToChip1:cCableToChip16+sCableToChip2:cCableToChip17+sCableToChip3:cCableToChip18+sCableToChip4:cCableToChip19+sCableToChip5:cCableToChip20");
    
    // -------Combine left ------------------
    TGeoCompositeShape *sCableCombL1 = new TGeoCompositeShape("sCableCombL1",
    "sCableStraitCombL1+sCableRoundCombL1+sCableToChipCombL1");
    TGeoCompositeShape *sCableCombL2 = new TGeoCompositeShape("sCableCombL2",
    "sCableStraitCombL2+sCableRoundCombL2+sCableToChipCombL2");

    //--------Combine All Cables----------------------
    TGeoCompositeShape *sCableComb = new TGeoCompositeShape("sCableComb", 
    "sCableCombL1+sCableCombL1:cCableStraitCombR1+sCableCombL2+sCableCombL2:cCableStraitCombR2+sCableCombR1+sCableCombR1:cCableStraitCombR1+sCableCombR2+sCableCombR2:cCableStraitCombR2");
    TGeoVolume *vCableComb = new TGeoVolume("vCableComb", sCableComb, medCables);
    vCableComb->SetLineColor(kYellow);
    vSector->AddNode(vCableComb, 1);   
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void    CreateWheel(TGeoVolumeAssembly *vSector, TGeoMedium *medAl)
{
...
//------------------ Korpus ----------------------------------------------------------------------------
    ...
    TGeoVolume *vKorp2 = new TGeoVolume("tof1Korp2", sKorp2, medAl);
    vKorp2->SetLineColor(kMagenta);
    ...
    TGeoVolume *vOS1 = new TGeoVolume("tof1OS1", sOS1, medAl);
    ...
    TGeoVolume *vOS2 = new TGeoVolume("tof1OS2", sOS2, medAl);
    ...
    TGeoVolume *vKoleso = new TGeoVolume("tof1Koleso", sKoleso, medAl);
    ...
    const size_t nMainKorpus = 20; TGeoCombiTrans *cMainKorpusTrans[nMainKorpus];
    ...
    cMainKorpusTrans[0] = new TGeoCombiTrans("cMainKorpusTrans1",   33.3+2.35/2, 1.85+1.13+0.75+2.55, -3.21 - 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[1] = new TGeoCombiTrans("cMainKorpusTrans2",  -33.3-2.35/2, 1.85+1.13+0.75+2.55,  3.21 + 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[2] = new TGeoCombiTrans("cMainKorpusTrans3",   33.3+2.35/2, 1.85+1.13+0.75+2.55,  3.21 + 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[3] = new TGeoCombiTrans("cMainKorpusTrans4",  -33.3-2.35/2, 1.85+1.13+0.75+2.55, -3.21 - 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[4] = new TGeoCombiTrans("cMainKorpusTrans5",   33.3+2.35/2, 1.85+1.13+0.75+2.55, -33.17 - 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[5] = new TGeoCombiTrans("cMainKorpusTrans6",  -33.3-2.35/2, 1.85+1.13+0.75+2.55,  33.17 + 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[6] = new TGeoCombiTrans("cMainKorpusTrans7",   33.3+2.35/2, 1.85+1.13+0.75+2.55,  33.17 + 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[7] = new TGeoCombiTrans("cMainKorpusTrans8",  -33.3-2.35/2, 1.85+1.13+0.75+2.55, -33.17 - 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[8] = new TGeoCombiTrans("cMainKorpusTrans9",   33.3+2.35/2, 1.85+1.13+0.75+2.55, -93.09 - 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[9] = new TGeoCombiTrans("cMainKorpusTrans10", -33.3-2.35/2, 1.85+1.13+0.75+2.55,  93.09 + 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[10]= new TGeoCombiTrans("cMainKorpusTrans11",  33.3+2.35/2, 1.85+1.13+0.75+2.55,  93.09 + 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[11]= new TGeoCombiTrans("cMainKorpusTrans12", -33.3-2.35/2, 1.85+1.13+0.75+2.55, -93.09 - 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[12]= new TGeoCombiTrans("cMainKorpusTrans13",  33.3+2.35/2, 1.85+1.13+0.75+2.55, -182.96 - 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[13]= new TGeoCombiTrans("cMainKorpusTrans14", -33.3-2.35/2, 1.85+1.13+0.75+2.55,  182.96 + 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[14]= new TGeoCombiTrans("cMainKorpusTrans15",  33.3+2.35/2, 1.85+1.13+0.75+2.55,  182.96 + 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[15]= new TGeoCombiTrans("cMainKorpusTrans16", -33.3-2.35/2, 1.85+1.13+0.75+2.55, -182.96 - 5.75/2 + 0.3, geoMainKorpus); 
    cMainKorpusTrans[16]= new TGeoCombiTrans("cMainKorpusTrans17",  33.3+2.35/2, 1.85+1.13+0.75+2.55, -282.84 - 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[17]= new TGeoCombiTrans("cMainKorpusTrans18", -33.3-2.35/2, 1.85+1.13+0.75+2.55,  272.84 + 5.75/2 + 0.3, geoMainKorpus);
    cMainKorpusTrans[18]= new TGeoCombiTrans("cMainKorpusTrans19",  33.3+2.35/2, 1.85+1.13+0.75+2.55,  272.84 + 5.75/2 + 0.3, geoMainKorpus2);
    cMainKorpusTrans[19]= new TGeoCombiTrans("cMainKorpusTrans20", -33.3-2.35/2, 1.85+1.13+0.75+2.55, -282.84 - 5.75/2 + 0.3, geoMainKorpus);

	for(size_t i = 0; i < nMainKorpus; i++)
	{
		vSector->AddNode(vKorp2, 1, cMainKorpusTrans[i]);

		vSector->AddNode(vOS2, 1, new TGeoCombiTrans((*cMainKorpusTrans[i])*(*cOS1)));
		vSector->AddNode(vOS1, 2, new TGeoCombiTrans((*cMainKorpusTrans[i])*(*cOS2)));

		vSector->AddNode(vKoleso, 1, new TGeoCombiTrans((*cMainKorpusTrans[i])*(*cKoleso1)));
		vSector->AddNode(vKoleso, 1, new TGeoCombiTrans((*cMainKorpusTrans[i])*(*cKoleso2)));
	}
}
//---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
void    CreatePlates(TGeoVolumeAssembly *vSector, TGeoMedium *medAl, double SecBoxZshift, double SecBox2Zshift)
{
    // ---------------- MIDDLE PLATE ------------------
    const float sectorPlateX = 69.8 - 0.01, sectorPlateY = 0.07, sectorPlateZ = 291.2 - 0.01; //lsp777  
    ...
    // ---------------- MIDDLE PLATE 2 ------------------
    const float sectorPlateZ2 = 281.2 - 0.01; //lsp777
    ...
    const float sectorPlateLeftTier1X = 2.4 - 0.01, sectorPlateLeftTier1Y = 1.2 - 0.01, sectorPlateLeftTier1Z = 281.2+0.3*2+delta1;//lsp888
    const float sectorPlateLeftTier2X = 0.3 - 0.01, sectorPlateLeftTier2Y = 4.3 - 0.01, sectorPlateLeftTier2Z = 281.2-1.9*2+delta1 - 0.01;//lsp9999
    ...
    TGeoVolume *vsectorPlateLeftMid = new TGeoVolume("vsectorPlateLeftMid", ssectorPlateLeftMid, medAl);                
    vsectorPlateLeftMid->SetLineColor(kRed);
    ...
    TGeoVolume *vsectorPlateLeftTier2 = new TGeoVolume("vsectorPlateLeftTier2", ssectorPlateLeftTier2, medAl);       
    vsectorPlateLeftTier2->SetLineColor(kGreen);
    TGeoVolume *vsectorPlateLeftTier2Bot = new TGeoVolume("vsectorPlateLeftTier2Bot", ssectorPlateLeftTier2Bot, medAl); 
    vsectorPlateLeftTier2Bot->SetLineColor(kGreen);
    ...
    TGeoBBox *ssectorPlateBackTier2Bot = new TGeoBBox(65.9/2.+0.35, sectorPlateLeftTier2BotY/2., sectorPlateLeftTier2X/2.); 
    ssectorPlateBackTier2Bot->SetName("sectorPlateBackTier2Bot");
    ...
    TGeoVolume *vsectorPlateBackMid = new TGeoVolume("vsectorPlateBackMid", ssectorPlateBackMid, medAl);                
    vsectorPlateBackMid->SetLineColor(kYellow);
    ...
    TGeoVolume *vsectorPlateBackTier2 = new TGeoVolume("vsectorPlateBackTier2", ssectorPlateBackTier2, medAl);          
    vsectorPlateBackTier2->SetLineColor(kGreen);
    TGeoVolume *vsectorPlateBackTier2Bot = new TGeoVolume("vsectorPlateBackTier2Bot", ssectorPlateBackTier2Bot, medAl); 
    vsectorPlateBackTier2Bot->SetLineColor(kGreen);
    ...
    vSector->AddNode(vsectorPlateBackMid, 1, new TGeoTranslation("tsectorPlateBackMid", 0., 1.85, -33.1-1.95-221.4/2. +SecBoxZshift -0.01));//X CHECK ! //overlap_lsp333_55555
    ...
    vSector->AddNode(vsectorPlateBackTier2, 1, new TGeoTranslation("tsectorPlateBackTier2", 0., 5.7, -31.-1.95-0.2-221.4/2. +SecBoxZshift -0.01));//overlap_lsp111_55555
    vSector->AddNode(vsectorPlateBackTier2Bot, 1, new TGeoTranslation("tsectorPlateBackTier2Bot", 0., -3.85, -31.-1.95-0.2-221.4/2. +SecBoxZshift -0.01));//overlap_lsp222_55555
    ...
    const float sectorPlateLeftTier1X2 = 2.4 - 0.01, sectorPlateLeftTier1Y2 = 1.2 - 0.01, sectorPlateLeftTier1Z2 = 281.2+0.3*2 -0.01;//overlap_lsp9999
    ...
    TGeoVolume *vsectorPlateLeftMid2 = new TGeoVolume("vsectorPlateLeftMid2", ssectorPlateLeftMid2, medAl);                 
    vsectorPlateLeftMid2->SetLineColor(kGreen);
    TGeoVolume *vsectorPlateLeftTier12 = new TGeoVolume("vsectorPlateLeftTier12", ssectorPlateLeftTier12, medAl);           
    vsectorPlateLeftTier12->SetLineColor(kGreen);
    ...
    vSector->AddNode(vsectorPlateLeftMid2, 1, new TGeoTranslation("tsectorPlateLeftMid2",  33.1+1.95+0.06, 1.85, 0. +SecBox2Zshift));//Z CHECK !//////////////////// 33.1+1.95
    ...
    vSector->AddNode(vsectorPlateLeftTier3Bot2, 2, new TGeoTranslation("tsectorPlateLeftTier32", 30.1+1.95+0.4, -8., -delta3/2.+SecBox2Zshift));
    //reverse (x->-x) for right side
    ...
    TGeoBBox *ssectorPlateBackTier22 = new TGeoBBox(65.9/2.+0.35 - 0.01, sectorPlateLeftTier2Y2/2. - 0.01, sectorPlateLeftTier2X2/2. -0.01); //overlap_121212
    ssectorPlateBackTier22->SetName("sectorPlateBackTier22");
...
void    CreatePowerFrame(TGeoVolumeAssembly *top, TGeoMedium *medAl)
{
    ...
    auto sOuterTube = new TGeoTube(  "sOuterTube", OuterTubeMin, OuterTubeMax, OuterTubeZ/2);
    auto vOuterTube = new TGeoVolume("vOuterTube", sOuterTube, medAl);
    vOuterTube->SetLineColor(kBlue);
   /// top->AddNode(vOuterTube, 1); //overlaps  tof1_0/vOuterTube_1 overlapping emcChamber_0 //lsp123123

    // Inner Tube
    const float InnerTubeMin = 168.0, InnerTubeMax = 170.25, InnerTubeZ = 624.4; 
    auto sInnerTube = new TGeoTube(  "sInnerTube", InnerTubeMin, InnerTubeMax, InnerTubeZ/2);
    auto vInnerTube = new TGeoVolume("vInnerTube", sInnerTube, medAl);
    vInnerTube->SetLineColor(kBlue);
  ///  top->AddNode(vInnerTube, 1); //overlaps tof1_0/vInnerTube_1 overlapping emcChamber_0  //lsp123123
    ...
    TGeoVolume *vBigRail = new TGeoVolume("tof1BigRail", sBigRail, medAl);
    vBigRail->SetLineColor(kGreen);
    TGeoRotation *rBigRail = new TGeoRotation("rBigRail", 0, 0, 180);

    top->AddNode(vBigRail, 1, new TGeoCombiTrans( InnerTubeMin, 0, 0, rBigRail) );
    top->AddNode(vBigRail, 2, new TGeoTranslation( -(InnerTubeMin), 0, 0) );

    // Small Rails
    const float SmallRailY = 3.59;
    TGeoTrd1 *sSmallRailBody = new TGeoTrd1("sSmallRailBody", 4.1/2, 5.9/2, 600/2, 3.59/2);
    TGeoVolume *vSmallRailBody = new TGeoVolume("tof1SmallRailBody", sSmallRailBody, medAl);
    ...
    TGeoVolume *vSmallRail = new TGeoVolume("tof1SmallRail", sSmallRaill, medAl);
    ...
    for(int i = 0; i < 12; i++)
    {
        ...
        top->AddNode(vSmallRail, i+1, RailCombi);
    }

    //------------------ Clock Face --------------------
    const float TrapezX_1 = OuterTubeZ+60, TrapezX_2 = InnerTubeZ - 1., TrapezY = 0.195, TrapezZ = OuterTubeMin - InnerTubeMax - 0.02; // lsp234234
    ...
    TGeoVolume *vTrapez = new TGeoVolume("tof1Trapez", sTrapez, medAl);
    ...
    for(int i = 0; i < 25; i++)
    {
     ...
     //   top->AddNode(vTrapez, i+1, TrapezCombi);  // tof1_0/tof1Trapez_9 overlapping emcChamber_0 //lsp123123
    }
...
/*    const float ToothRmin = 227.8, ToothRmax = 229.3, ToothZ = 40, Toothf1 = 90-4.734, Toothf2 = 94.734;
    ...
    TGeoVolume *vTooth = new TGeoVolume("tof1Tooth", sTooth0, medAl);
