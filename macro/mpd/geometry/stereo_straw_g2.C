...
void stereo_straw_g2() {

ostringstream fname; 
fname << "stereo_straw_" << numLayers << "_layers_" << Int_t(floor(innerRadius)) << "_x_" << Int_t(floor(outerRadius)) << ".02.geo";
...
TVector3 getPosition(Double_t angle) {
    Double_t r = innerRadius+(outerRadius-innerRadius)/2.;
