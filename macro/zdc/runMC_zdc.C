...
runMC_zdc (const char *datadir="")
{
  ...
  TString outFile = the_datadir+ Form("/zdc_%devents.root",nEvents);             
...
#define URQMD
  ...
  gROOT->LoadMacro("$VMCWORKDIR/macro/mpd/geometry_v2_minimal.C");  // macro for mini-set
  ...
  //  geometry_v2_option (fRun, kTRUE, "ZDC","zdc_modules84_layers60_16_4.geo");  
  // geometry_v2_minimal(fRun);                     // load minimal set of detector geometries
  //                                                //  (cave+pipe+magnet+tpc+tof)
  geometry_v2_minimal(fRun);                     // load minimal set of detector geometries
                                                 //  (cave+pipe+magnet+tpc+zdc)
