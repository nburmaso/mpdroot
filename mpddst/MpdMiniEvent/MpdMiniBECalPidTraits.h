 ...
 * \brief Holds information about ECal-matched tracks
 *
...
class MpdMiniBECalPidTraits: public TObject {
 ...
 public:
  ...
  MpdMiniBECalPidTraits(Int_t index, Int_t id, Int_t adc0, const Float_t* e,
			const Float_t* dist, const Int_t* ntow);
  /// Copy constructor
  ...
  /// Set BEMC ID
  void setBECalId(Int_t id)       { mBecalId = (id > std::numeric_limits<short>::max()) ? -1 : (Short_t)id; }
  /// Set ADC
 ...
 private:
  ...
  /// Next variables are extracted from the standard ECal cluster algorithm
  ...
  ClassDef(MpdMiniBECalPidTraits, 1);
};
