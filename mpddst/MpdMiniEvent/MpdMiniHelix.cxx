//
// MpdMiniHelix is a helix parametrization that uses ROOT TVector3
//
...
ClassImpT(MpdMiniHelix,double);

const Double_t MpdMiniHelix::NoSolution = 3.e+33;

//_________________
MpdMiniHelix::MpdMiniHelix() : fSingularity(false), fOrigin(0, 0, 0),
			       fDipAngle(0), fCurvature(0), fPhase(0),
			       fH(0), fCosDipAngle(0), fSinDipAngle(0),
			       fCosPhase(0), fSinPhase(0) {
  /*no-op*/
}

//_________________
MpdMiniHelix::MpdMiniHelix(Double_t c, Double_t d, Double_t phase,
			   const TVector3& o, Int_t h) {
  setParameters(c, d, phase, o, h);
}

//_________________
MpdMiniHelix::MpdMiniHelix(const MpdMiniHelix &h) {
  fSingularity = h.fSingularity;
  fOrigin = h.fOrigin;
  fDipAngle = h.fDipAngle;
  fCurvature = h.fCurvature;
  fPhase = h.fPhase;
  fH = h.fH;
  fCosDipAngle = h.fCosDipAngle;
  fSinDipAngle = h.fSinDipAngle;
  fCosPhase = h.fCosPhase;
  fSinPhase = h.fSinPhase;
}

//_________________
MpdMiniHelix::~MpdMiniHelix() { /* noop */ };

//_________________
void MpdMiniHelix::setParameters(Double_t c, Double_t dip, Double_t phase,
				 const TVector3& o, Int_t h) {
  ...
  // May change fH and fPhase. Must therefore be set last.
  //
  setCurvature(c);
...
//_________________
void MpdMiniHelix::setCurvature(Double_t val) {
  ...
  if ( ::fabs(fCurvature) <= numeric_limits<Double_t>::epsilon() ) {
#else
  if ( ::fabs(fCurvature) <= static_cast<Double_t>(0) ) {
#endif
...
//_________________
void MpdMiniHelix::setPhase(Double_t val) {
...
//_________________
void MpdMiniHelix::setDipAngle(Double_t val) {
...
//_________________
Double_t MpdMiniHelix::xcenter() const {
...
//_________________
Double_t MpdMiniHelix::ycenter() const {
  if (fSingularity) {
    return 0;
  }
  else {
...
//_________________
Double_t MpdMiniHelix::fudgePathLength(const TVector3& p) const {
  
  Double_t s;
  Double_t dx = p.x()-fOrigin.x();
  Double_t dy = p.y()-fOrigin.y();
...
//_________________
Double_t MpdMiniHelix::distance(const TVector3& p, Bool_t scanPeriods) const {
  return ( this->at( pathLength(p,scanPeriods) )-p ).Mag();
}

//_________________
Double_t MpdMiniHelix::pathLength(const TVector3& p, Bool_t scanPeriods) const {
  ...
  if (fSingularity) {
...
#ifndef ST_NO_NUMERIC_LIMITS
    {
      ...
      //
      Double_t t34 = fCurvature*fCosDipAngle*fCosDipAngle;
      Double_t t41 = fSinDipAngle*fSinDipAngle;
...
//_________________
Double_t MpdMiniHelix::period() const {
  if (fSingularity) {
#ifndef ST_NO_NUMERIC_LIMITS
...
//_________________
pair<Double_t, Double_t> MpdMiniHelix::pathLength(Double_t r) const {
  ...
  //
  if (fSingularity) {
    Double_t t1 = fCosDipAngle*(fOrigin.x()*fSinPhase-fOrigin.y()*fCosPhase);
    Double_t t12 = fOrigin.y()*fOrigin.y();
    Double_t t13 = fCosPhase*fCosPhase;
    Double_t t15 = r*r;
    Double_t t16 = fOrigin.x()*fOrigin.x();
    Double_t t20 = -fCosDipAngle*fCosDipAngle*(2.0*fOrigin.x()*fSinPhase*fOrigin.y()*fCosPhase +
					       t12-t12*t13-t15+t13*t16);
    if (t20<0.) {
      return VALUE;
    }
    t20 = ::sqrt(t20);
    value.first  = (t1-t20)/(fCosDipAngle*fCosDipAngle);
    value.second = (t1+t20)/(fCosDipAngle*fCosDipAngle);
  }
  else {
    Double_t t1 = fOrigin.y()*fCurvature;
    Double_t t2 = fSinPhase;
    Double_t t3 = fCurvature*fCurvature;
    Double_t t4 = fOrigin.y()*t2;
    Double_t t5 = fCosPhase;
    Double_t t6 = fOrigin.x()*t5;
    Double_t t8 = fOrigin.x()*fOrigin.x();
    Double_t t11 = fOrigin.y()*fOrigin.y();
    Double_t t14 = r*r;
    Double_t t15 = t14*fCurvature;
    Double_t t17 = t8*t8;
    ...
    Double_t t35 = t14*t3;
    Double_t t38 = 8.0*t4*t6 - 4.0*t1*t2*t8 - 4.0*t11*fCurvature*t6 +
    ...
    Double_t t43 = fOrigin.x()*fCurvature;
    Double_t t45 = 2.0*t5 - t35 + t21 + 2.0 - 2.0*t1*t2 -2.0*t43 - 2.0*t43*t5 + t8*t3;
    Double_t t46 = fH*fCosDipAngle*fCurvature;
	
    value.first = (-fPhase + 2.0*atan((-2.0*t1 + 2.0*t2 + t40)/t45))/t46;
    value.second = -(fPhase + 2.0*atan((2.0*t1 - 2.0*t2 + t40)/t45))/t46;
...
//_________________
pair<Double_t, Double_t> MpdMiniHelix::pathLength(Double_t r, Double_t x, Double_t y) {
  Double_t x0 = fOrigin.x();
  Double_t y0 = fOrigin.y();
  fOrigin.SetX(x0-x);
  fOrigin.SetY(y0-y);
...
//________________
Double_t MpdMiniHelix::pathLength(const TVector3& r,
  ...
  if (fSingularity) {
    ...
    Double_t A = fCurvature*((fOrigin - r)*n) -
      n.x()*fCosPhase - 
      n.y()*fSinPhase;
    Double_t t = fH*fCurvature*fCosDipAngle;
    Double_t u = n.z()*fCurvature*fSinDipAngle;
    ...
    Double_t deltas = fabs(angMax/(fCurvature*fCosDipAngle));
...
//_________________
pair<Double_t, Double_t> MpdMiniHelix::pathLengths(const MpdMiniHelix& h, Double_t minStepSize,
  ...
  if (fSingularity) {
    ...
    //  Analytic solution
    //
    TVector3 dv = h.fOrigin - fOrigin;
    TVector3 a(-fCosDipAngle*fSinPhase,
	       fCosDipAngle*fCosPhase,
	       fSinDipAngle);
    TVector3 b(-h.fCosDipAngle*h.fSinPhase,
...
//_________________
void MpdMiniHelix::moveOrigin(Double_t s) {
...
//_________________
Int_t operator== (const MpdMiniHelix& a, const MpdMiniHelix& b) {
...
//_________________
std::ostream& operator<<(std::ostream& os, const MpdMiniHelix& h) {
  return os << '('
