...
//------------------------------------------------------------------------------------------------------------------------
class MpdDch : public FairDetector
{
...
public:
  	...
  	MpdDch(const char* name, Bool_t active);

	MpdDch();
	virtual ~MpdDch();

	// Defines the action to be taken when a step is inside the
	// active volume. Creates MpdDchPoints and adds them to the collection.
...
private:
	...
	// Adds a MpdDchPoint to the HitCollection
  	MpdDchPoint* AddHit(Int_t trackID, Int_t detID, TVector3 pos, Double_t radius, TVector3 mom, Double_t time, 
...
//------------------------------------------------------------------------------------------------------------------------
inline void MpdDch::ResetParameters() 
{
	fTrackID = -1;
	fVolumeID = fwheel = 0;
