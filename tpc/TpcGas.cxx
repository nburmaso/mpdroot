    ...
    :_E(E)
{
  ...
  if (!infile.good())
    Fatal("TpcGas::TpcGas","Input File is not found");
  //ReadGasBegin returns the number of electric fields (and sets _T, _B, _p)
  int noent = ReadGasBegin(&infile);
  if (noent <= 0)
    Fatal("TpcGas::TpcGas","Number of electric fields nonpositive?\nCould not read File.\nExecution aborted.");
  ...
  int nCSDFile = ReadGasArrays(&infile, noent,
				e, vdrift, dt, dl, k);
  if (nCSDFile < 2)
    Fatal("TpcGas::TpcGas","Number of cluster sizes too small in File.\nExecution aborted.");
  ...
  if (!infile.good())
    Fatal("TpcGas::TpcGas","Could not read cluster size distribution in File.\nExecution aborted.");
  infile.close();
  //file is read
  ...
  if (inTable < 0 || inTable > noent -1)
    Warning("TpcGas::TpcGas","E field out of the range defined in input file");
...
void TpcGas::operator=(const TpcGas& GasToCopy)
{
...
int
TpcGas::GetRandomCSUniform() const 
{
...
void
TpcGas::SetCSD(const std::vector<double>& CSD)
{
  _CSD=CSD;
}

std::ostream& operator<< (std::ostream& stream, const TpcGas& g)
{
  stream << "--------------------------------------------------------\n"
	 << "Tpc gas parameters: \n"
...
int
TpcGas::ReadGasBegin(std::ifstream* const pinfile)
{
...
int
TpcGas::ReadGasArrays(std::ifstream* const pinfile, int const noent,
