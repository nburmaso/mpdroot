...
void MpdTpcFastDigitizer::Exec(Option_t* opt) 
{
  ...
  for (Int_t ip = 0; ip < nPoints; ++ip) {
    ...
    //Int_t isec = point->GetDetectorID() / 100 - 1;
...
void MpdTpcFastDigitizer::GetArea(Float_t xEll, Float_t yEll, Float_t radius, vector<UInt_t> &padIDs, vector<UInt_t> &rowIDs) 
{
...
Float_t MpdTpcFastDigitizer::CalculatePadResponse(Int_t iloop, Int_t nLoop, UInt_t padID, UInt_t rowID, Float_t x, Float_t y) 
{
...
void MpdTpcFastDigitizer::TpcProcessing(const TpcPoint* prePoint, const TpcPoint* curPoint, 
					const Int_t secID, const Int_t iPoint, const Int_t nPoints) 
{
  // Process one step  
  ...
  if ( fPrintDebugInfo && (iPoint % 1000 == 0) ) cout << Int_t(iPoint * 1.0 / nPoints * 100.0) << " % of TPC points processed" << endl;
  ...
  if (curPoint->GetTrackID() == prePoint->GetTrackID() && !isSubtrackInInwards(prePoint, curPoint)) {
    ...
    if (dE < 0) {
      Error("MpdTpcFastDigitizer::Exec", "Negative Energy loss!");
      return;
    }
    ...
    curPointPos.SetXYZT(curPoint->GetXout(), curPoint->GetYout(), curPoint->GetZout(), curPoint->GetTime());
    prePointPos.SetXYZT(prePoint->GetX(), prePoint->GetY(), prePoint->GetZ(), prePoint->GetTime());
    if ((curPointPos.T() < 0) || (prePointPos.T() < 0)) {
      Error("MpdTpcFastDigitizer::Exec", "Negative Time!");
      return;
    }
    ...
    //cout << " aaaa step " << diffPointPos.Vect().Mag() << " " << clustArr.size() << " " << diffPointPos.Vect().Mag()/clustArr.size() << endl;
    diffPointPos *= (1./ clustArr.size());
    clustPos = prePointPos;
    clustPos -= (0.5*diffPointPos); //AZ

    for (Int_t iClust = 0; iClust < clustArr.size(); ++iClust) {
      clustPos += diffPointPos;
      driftl = zCathode - fabs(clustPos.Z());
      if (driftl <= 0) continue;

      for (Int_t iEll = 0; iEll < clustArr.at(iClust); ++iEll) {
	...
	*/
	...
	PadResponse(localX, localY, curTimeID, origin, fDigits4dArray);
      } // for (Int_t iEll = 0; iEll < clustArr.at(iClust);
    } // for (Int_t iClust = 0; iClust < clustArr.size();
