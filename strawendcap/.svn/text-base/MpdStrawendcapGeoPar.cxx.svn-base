...
ClassImp(MpdStrawendcapGeoPar)

MpdStrawendcapGeoPar::MpdStrawendcapGeoPar(const char* name,const char* title,const char* context)
...
MpdStrawendcapGeoPar::~MpdStrawendcapGeoPar(void) {
}

void MpdStrawendcapGeoPar::clear(void) {
    if(fGeoSensNodes) delete fGeoSensNodes;
    if(fGeoPassNodes) delete fGeoPassNodes;
    fGeoSensNodes = fGeoPassNodes = 0x0; //AZ
}

void MpdStrawendcapGeoPar::putParams(FairParamList* l) {
  if (!l) return;
...
Bool_t MpdStrawendcapGeoPar::getParams(FairParamList* l) {
    if (!l) return kFALSE;
