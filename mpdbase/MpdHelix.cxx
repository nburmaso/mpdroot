...
MpdHelix::MpdHelix() : mSingularity(false),
                       mOrigin(),
                       mDipAngle(0),
                       mCurvature(0),
                       mPhase(0),
...
pair<double, double> MpdHelix::pathLength(double r) const
{
  ...
  if (mSingularity) {
    ...
    double t20 = -mCosDipAngle * mCosDipAngle * (2.0 * mOrigin.X() * mSinPhase * mOrigin.Y() * mCosPhase + t12 - t12 * t13 - t15 + t13 * t16);
    if (t20 < 0.)
      return VALUE;
    t20 = ::sqrt(t20);
  ...
  MpdHelix::pathLengths(const MpdHelix& hh) const
{
  ...
  if (mSingularity) {
    ...
    if (fabs(cosAlpha) < 1) { // two solutions
      ...
      if (hh.distance(at(a)) < hh.distance(at(s)))
        s = a;
    ...
    const double MinStepSize = 1e-3;
    const double MinRange = 50;
    double dmin = hh.distance(at(s), true);
    printf("dmin = %f\n", dmin);
