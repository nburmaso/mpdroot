...
C----  FOR MESON-MESON COLLISION 
      ...
      END 
C * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * * 
      SUBROUTINE HADGENANG(TFOR,TBACK,T,Z,PHI) 
      IMPLICIT REAL*8 (A-H,O-Z) 
...
C--------------- HADES2 -----------------------------
      ...
      if(IK.le.0)then
        ISLERR=1
        CONTINUE!WRITE(25,10)IK,NUMTRE
...
C The diagnostic printout is modified by Sobol, 16.04.2002    ! sobol
      IF(L.GT.50)CONTINUE!WRITE(25,1001)L,S                            ! sobol
