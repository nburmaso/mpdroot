...
class NicaTpcSectorGeo : public TObject {
 ...
 public:
  static NicaTpcSectorGeo *Instance(
      TString geo = "full");  ///< get singleton instance
  Int_t Global2Local(const Double_t *xyzGlob, Double_t *xyzLoc,
                     Int_t iSec = -1);  ///< transform global coordinates to
                                        ///< local (sector) - returns padID
  Int_t Global2Local(const TVector3 &xyzGlob, TVector3 &xyzLoc,
                     Int_t iSec = -1);  ///< transform global coordinates to
                                        ///< local (sector) - returns padID
  void Local2Global(Int_t iSec, const Double_t *xyzLoc,
                    Double_t *xyzGlob);  ///< transform local coordinates of
                                         ///< sector iSec to global
  void Local2Global(Int_t iSec, const TVector3 &xyzLoc,
                    TVector3 &xyzGlob);  ///< transform local coordinates of
                                         ///< sector iSec to global
  Int_t PadRow(Int_t padID) {
    return (padID >> kPadrowS) & kPadrowM;
  }  ///< pad rows from padID
  Int_t Sector(Int_t padID) {
    return (padID >> kSectorS) & kSectorM;
  }  ///< sector No. from padID
  Int_t PadID(Int_t sec, Int_t row) {
    return sec |= (row << kPadrowS);
  }  ///< padID from sector and padrow numbers
  void PadID(Float_t xloc, Float_t yloc, UInt_t &row, UInt_t &pad,
             Float_t &yNext);  ///< get row, pad and distance to nearest row
  ...
  Double_t GetRocY(Int_t i) {
    return fYsec[i];
  }  ///< get ROC region Y-coordinates
  Double_t PadHeight(Int_t ireg = 0) {
    return fPadH[ireg];
  }  ///< get pad height in ROC region ireg
  Double_t PadWidth(Int_t ireg = 0) {
    return fPadW[ireg];
  }  ///< get pad width in ROC region ireg
  Double_t SectorAngle(Int_t iSec) {
    return fPhi0 + (iSec + 0.5) * fDphi;
  }                                  ///< get azimuthal angle of sector axis
  Int_t GetZmin() { return fZmin; }  ///< sensitive volume Zmin
  Int_t GetZmax() { return fZmax; }  ///< sensitive volume Zmax
  const Int_t *NPadsInRows() const {
    return fNPadsInRows;
  }  ///< numbers of pads in rows
  Double_t Pad2Xloc(Double_t pad,
                    Int_t row);  ///< Pad number-to-Xlocal conversion
 ...
 protected:
  // virtual void Finish();
  NicaTpcSectorGeo(TString geo = "full") : fSectAngle(TMath::Pi() / 12.0) {
    Init(geo);
  }  ///< Default ctor
  Bool_t GetPad(const NicaHelix *helix, Double_t R, Double_t &S, Int_t &Sect);
  Bool_t CheckSector(const NicaHelix *helix, Double_t R, Double_t &S,
                     Int_t Sect);
  Bool_t AdditionalCheck(const NicaHelix *helix, Double_t S, Double_t R,
                         Double_t sect) const;
  virtual ~NicaTpcSectorGeo() { ; }  ///< Destructor
  Int_t FindFirstPad(const NicaHelix *helix, Float_t *Paths, Int_t &sect);
  Int_t FindLastPad(const NicaHelix *helix, Float_t *Paths, Int_t first_pad,
                    Int_t &sect);
 ...
 private:
  ...
  Double_t fYsec[3];  // coordinates of ROC regions (height direction): minimum,
                      // boundary, maximum
