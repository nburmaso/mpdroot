...
void NicaPairAnaDebugger::ProcessEvent() {
	...
	NicaCutCollection *cont = fCutContainer->GetEventCollection(fCurrentEventCollectionID);
	for(int i=0;i<fMemoryMap->GetTemporaryTotalTracksNo();i++){
		...
		for(int j=0;j<cont->GetNextNo();j++){
			...
			if(fCutContainer->PassTrack(fCurrentTrack,fCurrentTrackCollectionID)){
				fMemoryMap->AddTrackToMapTrack(fCurrentEventCollectionID,fCurrentTrackCollectionID,i);//load track into memory map - may be usefull at finish event
				ProcessTrack();
				NicaComplexTrack *t = (NicaComplexTrack*)fCurrentTrack;
	//			std::cout<<"TR"<<t->GetRealTrack()->GetPx()<<" "<<t->GetImgTrack()->GetPx()<<std::endl;
	//			std::cout<<"TR"<<t->GetRealTrack()<<" "<<t->GetImgTrack()<<std::endl;
			}
		}
	}
	...
	for(int i=0;i<nTrack;i++){
		track = fMemoryMap->GetTrack(fCurrentEventCollectionID,fCurrentTrackCollectionID,i);
		NicaComplexTrack *cpz = (NicaComplexTrack*)track;
		std::cout<<" NMX "<<std::endl;
		std::cout<<"\t"<<cpz->GetRealTrack()->GetPx()<<" "<<cpz->GetImgTrack()->GetPx()<<std::endl;
		std::cout<<"\t"<<cpz->GetRealTrack()<<" "<<cpz->GetImgTrack()<<std::endl;
	}
//return;
	NicaCutCollection *track_cuts = fCutContainer->GetTrackCollection(fCurrentTrackCollectionID);
	...
	if(fMemoryMap->IsReadyToMixing(fCurrentEventCollectionID)){
		for(int l = 0;l<fMixSize;l++){
			...
			for(int i = 0;i<nTrackA;i++){
				tr1 = (NicaComplexTrack*)fMemoryMap->GetTrack(fCurrentEventCollectionID,fCurrentTrackCollectionID,i);
				for(int j=0;j<nTrackB;j++){
					tr2 =(NicaComplexTrack*)fMemoryMap->GetTrack(fCurrentEventCollectionID,fCurrentTrackCollectionID,l,j);
					tr1->ClassName();
					tr2->ClassName();
				}
			}
