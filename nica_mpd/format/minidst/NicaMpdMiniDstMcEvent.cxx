...
NicaMpdMiniDstMcEvent::NicaMpdMiniDstMcEvent(
        const NicaMpdMiniDstMcEvent &other) :NicaMCEvent(other){
}
...
void NicaMpdMiniDstMcEvent::Update() {
    ...
    for(int i=0;i<tracks->GetEntriesFast();i++){
        ...
        mc->GetMomentum()->SetPxPyPzE(track->px(),track->py(), track->pz(), track->energy());
       // mc->SetMotherIndex(track->parentIndex());
        mc->SetMotherIndex(0);
        if(mc->GetMotherIndex()<0)
            mc->SetPrimary(kTRUE);
        else
            mc->SetPrimary(kFALSE);
        mc->SetCharge(CalculateCharge(track->pdgId()));
        mc->SetPdg(track->pdgId());
        mc->GetStartPosition()->SetXYZT(track->x(), track->y(), track->z(), track->t());
        mc->GetLink()->Clear();
...
void NicaMpdMiniDstMcEvent::Clear(Option_t *opt) {
    fTracks->Clear(opt);
}

void NicaMpdMiniDstMcEvent::Print() {
    NicaMCEvent::Print();
}
