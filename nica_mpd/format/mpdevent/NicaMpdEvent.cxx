...
NicaMpdEvent::NicaMpdEvent(const NicaMpdEvent &other)
    : NicaExpEventHelix(other) {
...
void NicaMpdEvent::Update() {
  ...
  fVertex->SetXYZT(event->GetPrimaryVerticesX(), event->GetPrimaryVerticesY(),
                   event->GetPrimaryVerticesZ(), 0);
  fVertexError->SetXYZT(event->GetPrimaryVerticesXerr(),
                        event->GetPrimaryVerticesYerr(),
                        event->GetPrimaryVerticesZerr(), 0);
  ...
  switch (fMode) {
    case kAllTracks: {
      ...
      for (int i = 0; i < event->GetEventInfoNofPrimaryTracks(); i++) {
        NicaMpdTrack *mpd_track =
            (NicaMpdTrack *)fTracks->ConstructedAt(count++);
        mpd_track->Update((MpdTrack *)prim->UncheckedAt(i));
        mpd_track->SetEvent(this);
        mpd_track->GetLink()->ClearLinks();
      ...
      for (int i = 0; i < event->GetEventInfoNofGlobalTracks(); i++) {
        ...
        mpd_track->SetEvent(this);
        mpd_track->GetLink()->ClearLinks();
        mpd_track->GetLink()->SetLink(
            0, i + event->GetEventInfoNofPrimaryTracks());
