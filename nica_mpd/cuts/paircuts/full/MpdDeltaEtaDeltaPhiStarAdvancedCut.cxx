...
Bool_t MpdDeltaEtaDeltaPhiStarAdvancedCut::Pass(NicaTwoTrack *pair) {
  ...
  SetValue(track1->GetMomentum()->Eta() - track2->GetMomentum()->Eta(),
           DeltaEta());
  phi_star_min = 1E+9;
  phi_star = 0;
  ...
  for (int i = 0; i < minHits; i++) {
    ...
    Double_t dPhi =
        (Phi2 - Phi1 + (TMath::ASin(-0.075 * charge2 * magSign * rad / ptv2)) -
  ...
  if (InLimits(DeltaEta()) && InLimits(DeltaPhiStar()) &&
      InLimits(DeltaPhiStarMin())) {
