...
fdX(0),fdY(0),fdZ(0), fFirstCommonPad(0),fLastCommonPad(0)
{

}

Double_t MpdNominalTpcPairDistanceCut::GetDistance(Int_t lay) const {
	Double_t s1 = fTrack1->GetPathAt(lay);
	NicaHelix *helix1 = fTrack1->GetHelix();
	/* manual calcuations instead of helix, speed */
	Double_t x1 = helix1->GetStartX()
			+ (TMath::Cos(helix1->GetPhi0() + s1 * helix1->GetH() * helix1->GetCurv() * fCosDipAngle1)
					- fCosPhase1) / helix1->GetCurv();
	Double_t y1 = helix1->GetStartY()
			+ (TMath::Sin(helix1->GetPhi0() + s1 * helix1->GetH() *  helix1->GetCurv() * fCosDipAngle1)
					- fSinPhase1) /  helix1->GetCurv();
	Double_t z1 =  helix1->GetStartZ() + s1 * fSinDipAngle1;
	Double_t s2 = fTrack2->GetPathAt(lay);
	NicaHelix *helix2 = fTrack2->GetHelix();
	Double_t x2 = helix2->GetStartX()
			+ (TMath::Cos(helix2->GetPhi0() + s2 * helix2->GetH() * helix2->GetCurv() * fCosDipAngle2)
					- fCosPhase2) / helix2->GetCurv();
	Double_t y2 = helix2->GetStartY()
			+ (TMath::Sin(helix2->GetPhi0() + s2 * helix2->GetH() *  helix2->GetCurv() * fCosDipAngle2)
					- fSinPhase2) /  helix2->GetCurv();
...
void MpdNominalTpcPairDistanceCut::InitPass(NicaTwoTrack* pair) {
	...
	const Int_t start1_pad = fTrack1->GetFirstGoodPad();
	const Int_t start2_pad = fTrack2->GetFirstGoodPad();
	const Int_t	end1_pad = fTrack1->GetFirstBadPad();
	const Int_t end2_pad = fTrack2->GetFirstBadPad();
...
MpdNominalTpcPairDistanceCut::MpdNominalTpcPairDistanceCut(const MpdNominalTpcPairDistanceCut& other) :
		...
		fSinPhase1(other.fSinPhase1),fSinPhase2(other.fSinPhase2),
		fTrack1(other.fTrack1),fTrack2(other.fTrack2),
		fdX(other.fdX),fdY(other.fdY),fdZ(other.fdZ),
...
MpdNominalTpcPairDistanceCut::~MpdNominalTpcPairDistanceCut() {
}

TVector3 MpdNominalTpcPairDistanceCut::GetDistance3D(Int_t lay) const {
	Double_t s1 = fTrack1->GetPathAt(lay);
	NicaHelix *helix1 = fTrack1->GetHelix();
	/* manual calcuations instead of helix, speed */
	Double_t x1 = helix1->GetStartX()
