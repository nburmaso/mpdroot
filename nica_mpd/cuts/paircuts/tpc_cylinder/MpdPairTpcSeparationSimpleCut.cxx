...
MpdPairTpcSeparationSimpleCut::MpdPairTpcSeparationSimpleCut(Int_t averaging_steps) :
...
Bool_t MpdPairTcpSeparationSimpleNegCut::Pass(NicaTwoTrack* pair) {
	...
	SetValue(max,Max());
	SetValue(min,Min());
	Double_t gMin = TMath::Min(GetValue(Entry()),TMath::Min(GetValue(Max()),GetValue(Average())));
	Double_t gMax = TMath::Max(GetValue(Entry()),TMath::Max(GetValue(Max()),GetValue(Average())));
	//SetValue(gMin,5);
	//SetValue(gMax,6);
