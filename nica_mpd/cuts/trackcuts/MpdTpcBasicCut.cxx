...
MpdTcpCalibratedCut::MpdTcpCalibratedCut(TString calib_file)
    : NicaTrackTpcCut(), fUseDst(kFALSE) {
  ...
  TString trash;
  calib >> trash >> trash;
  for (int i = 0; i < 6; i++) calib >> fPionBB[i];
  calib >> trash >> trash;
  for (int i = 0; i < 6; i++) calib >> fKaonBB[i];
  calib >> trash >> trash;
  for (int i = 0; i < 6; i++) calib >> fProtonBB[i];
  calib >> trash >> trash;
  for (int i = 0; i < 6; i++) calib >> fElectronBB[i];
  for (int i = 0; i < 7; i++) calib >> trash;
  for (int i = 0; i < 72; i++) calib >> fSigmasPi[i];
  for (int i = 0; i < 7; i++) calib >> trash;
  for (int i = 0; i < 72; i++) calib >> fSigmasK[i];
  for (int i = 0; i < 7; i++) calib >> trash;
  for (int i = 0; i < 72; i++) calib >> fSigmasP[i];
  for (int i = 0; i < 7; i++) calib >> trash;
  for (int i = 0; i < 72; i++) calib >> fSigmasE[i];
  for (int i = 0; i < 7; i++) calib >> trash;
  for (int i = 0; i < 72; i++) calib >> fPInt[i];
  calib.close();
}

Bool_t MpdTcpCalibratedCut::Pass(NicaTrack* track) {
  NicaTpcTrack* tpc =
      (NicaTpcTrack*)((NicaMpdTrack*)track)->GetDetTrack(MpdDetectorID::kTPC);
  ...
  if (fUseDst) {
    ...
    SetValue((dedx - BetheBlochFunction(mom, fPionBB)) / fSigmasPi[i_p],
             fgSigmaPionId);
    SetValue((dedx - BetheBlochFunction(mom, fKaonBB)) / fSigmasK[i_p],
             fgSigmaKaonId);
    SetValue((dedx - BetheBlochFunction(mom, fProtonBB)) / fSigmasP[i_p],
             fgSigmaProtonId);
    SetValue((dedx - BetheBlochFunction(mom, fElectronBB)) / fSigmasE[i_p],
             fgSigmaElectronId);
  }
