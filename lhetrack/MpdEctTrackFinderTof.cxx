...
void MpdEctTrackFinderTof::MergeWithTpc(MpdEctKalmanTrack *track)
{
    ...
    for (Int_t j = 0; j < nHits; ++j) {
        MpdKalmanHit *khit = (MpdKalmanHit*) hits->UncheckedAt(j);
        lay0 = lay;
        lay = khit->GetLayer();
        //if (!MpdKalmanFilter::Instance()->PropagateToHit(track,hit,kFALSE)) continue;
        ...
        if (stepMap.find(lay) != stepMap.end() && stepMap.find(lay0) != stepMap.end())
            stepBack = TMath::Abs (stepMap[lay] - stepMap[lay0]);
        if (!MpdKalmanFilter::Instance()->PropagateToHit(track,khit,kFALSE,kTRUE,stepBack)) {
            track->SetNodeNew(track->GetNode()); // restore node
            continue;
        }
        Double_t dChi2 = MpdKalmanFilter::Instance()->FilterHit(track,khit,pointWeight,param);
        //cout << j << " " << hit->GetDist() << " " << dChi2 << endl;
        if (dChi2 > fgkChi2Cut * 4) continue;
        //track->SetParam(param);
        track->SetParamNew(param);
        (*track->GetWeight()) += pointWeight;
        track->GetHits()->Add(khit);
        track->SetChi2(track->GetChi2()+dChi2);
        new ((*oldHits)[nOld++]) MpdKalmanHit (*khit);
        ++nAdd;
        // Set params at hit
        track->SetParamAtHit(*track->GetParamNew());
        track->SetWeightAtHit(*track->GetWeight());
...
void MpdEctTrackFinderTof::GoOutward()
{
    ...
    */
        ...
        for (Int_t jh = 0; jh < nHits; ++jh) {
            MpdKalmanHit *khit = (MpdKalmanHit*) trhits->UncheckedAt(jh);
            if (khit->GetType() == MpdKalmanHit::kFixedR || khit->GetType() == MpdKalmanHit::kFixedP) ++nTpc;
            else ++nEct;
        }
        //*
        ...
        if (nEct > 0) {
            ...
            for (Int_t jh = nEct - 1; jh >= 0; --jh) {
                MpdKalmanHit* khit = (MpdKalmanHit*) hits->UncheckedAt(jh);
                //if (!MpdKalmanFilter::Instance()->PropagateToHit(&tr,hit,kFALSE)) continue;
                if (!MpdKalmanFilter::Instance()->PropagateToHit(&tr,khit,kTRUE)) continue;
                PassWall(&tr, 0.001);
                Double_t dChi2 = MpdKalmanFilter::Instance()->FilterHit(&tr,khit,pointWeight,param);
                //tr.SetParam(param);
