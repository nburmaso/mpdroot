...
//__________________________________________________________________________
double MpdKfV0Fitter::EvalVertex(MpdKalmanTrack* tr[2])
{
  ...
  p1 += p2;
  p1 *= 0.5;
  if (TMath::Abs(paths.first) > 80)
    p1.SetXYZ(0, 0, 0);
  p1.GetXYZ(fVtx);
  p1.Print();
  return helix[0]->distance(helix[1]->at(paths.second));
  for (Int_t i = 0; i < 2; ++i)
    delete helix[i];
  // sum *= 0.5;
...
Double_t MpdKfV0Fitter::FindVertex(MpdKalmanTrack* tr1, MpdKalmanTrack* tr2, TVector3& vtx)
{
  ...
  double dca_2heli = EvalVertex(tracks);
...
void MpdKfV0Fitter::Smooth(MpdKalmanTrack* tr[2])
{
  ...
  for (Int_t itr = 0; itr < 2; ++itr) {
    ...
    track1.SetLength(0.);
    if (track->GetNode() == "")
      MpdKalmanFilter::Instance()->PropagateParamR(&track1, &hit, kTRUE);
    else
      MpdKalmanFilter::Instance()->PropagateParamP(&track1, &hit, kTRUE, kTRUE);
                                 ...
                                 TMatrixD& a, TMatrixD& b, TMatrixD& ck0)
{
  ...
  if (r > 1.e-7)
    phi = TMath::ATan2(vert0[1], vert0[0]);
  trackk.SetPos(r);
  ...
  if (track->GetNode() == "") {
    ...
    d += v3 * norm;
    if (d < 0)
      trackk.SetDirection(MpdKalmanTrack::kOutward);
  ...
  for (Int_t i = 0; i < 3; ++i) {
    ...
    if (r > 1.e-7)
      ...
      phi = track0.GetParamNew(2); // track Phi
    track1.SetPos(r);
    ...
    if (track->GetNode() == "") {
      ...
      MpdKalmanFilter::Instance()->PropagateParamP(&track1, &hit, kFALSE, kTRUE);
    // Derivatives
    for (Int_t j = 0; j < 5; ++j) {
  ...
  for (Int_t i = 0; i < 3; ++i) {
    ...
    if (j == 4)
      shift *= TMath::Sign(1., -track0.GetParamNew(j)); // 1/p
  ...
  TMatrixD qk0(3, 1);
  for (Int_t i = 0; i < 3; ++i)
    qk0(i, 0) = track0.GetParamNew(i + 2);
  // qk0.Print();
