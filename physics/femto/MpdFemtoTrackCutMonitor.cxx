...
#include "MpdFemtoTrackCutMonitor.h"
...
monMass(ParticleMass) {
    ...
    // Constructor
    string s("MpdFemtoTrackCutMonitor");
    string n(name);
...
MpdFemtoTrackCutMonitor::MpdFemtoTrackCutMonitor(const MpdFemtoTrackCutMonitor& cutMoni) :
...
MpdFemtoTrackCutMonitor MpdFemtoTrackCutMonitor::operator=(const MpdFemtoTrackCutMonitor& cutMoni) {
    // Assignment operator
    if (this != &cutMoni) {
        if (mDCAGlobal) delete mDCAGlobal;
        mDCAGlobal = new TH1F(*(cutMoni.mDCAGlobal));
...
MpdFemtoTrackCutMonitor::~MpdFemtoTrackCutMonitor() {
    if (mDCAGlobal) {
        delete mDCAGlobal;
        mDCAGlobal = nullptr;
    }
...
void MpdFemtoTrackCutMonitor::fill(const MpdFemtoTrack* track) {
...
void MpdFemtoTrackCutMonitor::writeOutHistos() {
...
TList* MpdFemtoTrackCutMonitor::getOutputList() {
