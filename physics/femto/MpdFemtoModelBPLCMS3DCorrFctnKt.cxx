...
MpdFemtoBaseCorrFctn() {
    ...
    for (int iHisto = 0; iHisto < 20; iHisto++) {
        ...
        if (mNumeratorWeighted[iHisto]) {
            delete mNumeratorWeighted[iHisto];
            mNumeratorWeighted[iHisto] = nullptr;
        }
        if (mDenominator[iHisto]) {
            delete mDenominator[iHisto];
            mDenominator[iHisto] = nullptr;
        }
        if (mDenominatorWeighted[iHisto]) {
            delete mDenominatorWeighted[iHisto];
            mDenominatorWeighted[iHisto] = nullptr;
        }
    } // for (int iHisto=0; iHisto<20; iHisto++ )
...
MpdFemtoModelBPLCMS3DCorrFctnKt::~MpdFemtoModelBPLCMS3DCorrFctnKt() {
    ...
    for (int i = 0; i < 20; i++) {
        ...
        if (mNumeratorWeighted[i]) {
            delete mNumeratorWeighted[i];
            mNumeratorWeighted[i] = nullptr;
        }
        if (mDenominator[i]) {
            delete mDenominator[i];
            mDenominator[i] = nullptr;
        }
        if (mDenominatorWeighted[i]) {
            delete mDenominatorWeighted[i];
            mDenominatorWeighted[i] = nullptr;
        }
    } // for (int i=0; i<20; i++)
}
