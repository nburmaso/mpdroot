...
class MpdFemtoBaseEventReader {
 ...
 public:
  ...
  virtual ~MpdFemtoBaseEventReader()  { /* empty */ }
  ...
  /// Set event cut
  virtual void setEventCut(MpdFemtoBaseEventCut* ecut) { mEventCut = ecut; }
  /// Set track cut
  virtual void setTrackCut(MpdFemtoBaseTrackCut* pcut) { mTrackCut = pcut; }
  /// Set v0 cut
  virtual void setV0Cut(MpdFemtoBaseV0Cut* pcut)       { mV0Cut = pcut; }
  /// Set xi cut
  virtual void setXiCut(MpdFemtoBaseXiCut* pcut)       { mXiCut = pcut; }
  /// Set kink cut
  virtual void setKinkCut(MpdFemtoBaseKinkCut* pcut)   { mKinkCut = pcut; }

  /// Return pointer to event cut
  virtual MpdFemtoBaseEventCut* eventCut() { return mEventCut; }
  /// Return pointer to track cut
  virtual MpdFemtoBaseTrackCut* trackCut() { return mTrackCut; }
  /// Return pointer to V0 cut
  virtual MpdFemtoBaseV0Cut*    v0Cut()    { return mV0Cut; }
  /// Return pointer to V0 cut
  virtual MpdFemtoBaseXiCut*    xiCut()    { return mXiCut; }
  /// Return pointer to kink cut
  virtual MpdFemtoBaseKinkCut*  kinkCut()  { return mKinkCut; }
