...
bool MpdFemtoBasicEventCut::pass(const MpdFemtoEvent* ev) {
    ...
    // Event variables
    // std::cout << "Event variables: \n"
    //     << Form("x: %4.2f y: %4.2f z: %4.2f \n", ev->primaryVertex().X(), ev->primaryVertex().Y(), ev->primaryVertex().Z() )
    //     << Form("vtzVz: %4.2f r: %4.2f \n", ev->vpdVzDiff(), vtxR )
    //     << Form("sph: %4.2f \n", ev->sphericity() )
    //     << Form("refMult: %d BTofMult: %d BTofMatch: %d \n", ev->refMult(), ev->btofTrayMultiplicity(), ev->numberOfBTofMatched() )
    //     << Form("Cent: %d Psi: %4.2f \n", ev->cent9(), ev->eventPlaneAngle() );
    ...
    // std::cout << "MpdFemtoBasicEventCut:: goodEvent " << goodEvent << std::endl
...
bool MpdFemtoBasicEventCut::isInBadRunList(int runNumber) {
    ...
    return ( isIn_7gev || isIn_11gev || isIn_14gev || isIn_19gev || isIn_27gev ||
            isIn_39gev || isIn_62gev || isIn_200gev);
}
