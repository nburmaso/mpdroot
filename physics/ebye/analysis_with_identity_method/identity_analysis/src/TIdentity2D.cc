...
void TIdentity2D::GetTree(Long_t &nent, TString idenTreeName)
{
    ...
    if (fTIdenFileName.Contains(".root"))
    {
        ...
        cout << "IA test: " << fTIdentityTree << endl;
    }
    else
    {
    ...
    { // new version of tree format
        ...
        fTIdentityTree->SetBranchAddress("event", &fEventNum);
//        fTIdentityTree->SetBranchAddress("dca", &fDEdx);
        fTIdentityTree->SetBranchAddress("dEdx", &fDEdx);
//        fTIdentityTree->SetBranchAddress("mom", &fMom);
//        fTIdentityTree->SetBranchAddress("dcaR", &fDEdx);
        fTIdentityTree->SetBranchAddress("sign", &fSign);
        //    fTIdentityTree->SetBranchAddress("cutBit", &fCutBit);
...
Bool_t TIdentity2D::GetEntry(Int_t i)
{
    ...
    // !! IA: take F or B into account:
//    fDEdx *= fSign;
    fSign = 0; // !! BY HAND
//    if ( /*fMom*/fBranchVariables[3] < 0 ) fDEdx*=-1; // BY HAND!!

//    cout << "fMom = " << /*fMom*/fBranchVariables[3] << ", fDEdx = " << fDEdx << endl;


    if (fMyBinBrach) fEventNum = fEventNumOldVersion;
//    cout << "before: fMyDeDx = " << fMyDeDx << endl;
//    double prev_fMyDeDx = fMyDeDx;
...
//    cout << "fMyDeDx before-after: " << prev_fMyDeDx << " " << fMyDeDx << endl;
    if ((fMyDeDx < fMindEdx || fMyDeDx > fMaxdEdx) || fMyDeDx == 0) { /*cout << "RETURN FALSE (1)!!!" << endl;*/ return kFALSE; }

    //
    // secure the usage of sign=0 which is sum of + and - particles
    //
    if (!(fSign == fUseSign || fUseSign == 0)) { cout << "RETURN FALSE (2)!!!" << endl; return kFALSE; }
    return kTRUE;
}
...
Int_t TIdentity2D::AddEntry()
{
    ...
    else
    {
        ...
        {
//            cout << "############" << endl;
//            cout << "fCountPart    = " << fCountPart    << endl;
//            cout << "fCountPartNeg = " << fCountPartNeg << endl;
//            cout << "fCountPartPos = " << fCountPartPos << endl;
...
{
//    cout << "test in AddParticles()" << endl;
...
//        cout << "in AddParticles(): fMyDeDx = " << fMyDeDx << endl;

        fMValue[i] = fTFunctions[i]->Eval(fMyDeDx);
        sumfMValue += fMValue[i];

//        cout << i << "   fSign=" << fSign << "     " << fMyDeDx << "     " << fMValue[i]  << endl;
    }
    //
